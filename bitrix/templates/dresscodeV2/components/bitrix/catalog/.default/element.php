...
<?
	...
	elseif($obCache->StartDataCache()){
		...
		if(!empty($arResult["VARIABLES"]["ELEMENT_CODE"]) && CModule::IncludeModule("iblock")){
			...
			// elseif(!empty($arResult["VARIABLES"]["SECTION_CODE_PATH"])){
...
<?$APPLICATION->IncludeComponent(
	...
	"detail",
	array(
		"PRODUCT_ID" => (intval($_REQUEST["offerID"]) > 0 ? intval($_REQUEST["offerID"]) :  (!empty($arResult["VARIABLES"]["ELEMENT_ID"]) ? $arResult["VARIABLES"]["ELEMENT_ID"] : (!empty($arResult["EXTRA"]["ID"]) ? $arResult["EXTRA"]["ID"] : "-"))),
