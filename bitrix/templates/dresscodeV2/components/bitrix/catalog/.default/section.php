...
<?
	...
	//get from cache
	if($obCache->InitCache(36000, serialize(array_merge(['SITE_ID'=>SITE_ID],$arFilter)), "/")){
	...
	elseif($obCache->StartDataCache()){
		...
		$arCurSection = array();
		$dbRes = CIBlockSection::GetList(array(), $arFilter, false, array("ID", "NAME", "IBLOCK_ID"));
...
?>
<h1><?=$APPLICATION->ShowTitle(false);?><?/*if(!empty($arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"])):?><?=$arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]?><?else:?><?=$arCurSection["NAME"]?><?endif;*/?></h1>
		...
		<ul class="slideBox">
			<?foreach($arResult["SECTION_BANNERS"] as $isc => $arNextBanner):?>
				<?if(!empty($arNextBanner["IMAGE"])):?>
					<li>
						<a<?if(!empty($arNextBanner["LINK"])):?> href="<?=$arNextBanner["LINK"]?>"<?endif;?> class="cat-slider">
							<?if(!empty($arParams["LAZY_LOAD_PICTURES"]) && $arParams["LAZY_LOAD_PICTURES"] == "Y"):?>
								<img src="<?=$templateFolder?>/images/lazy.jpg" data-lazy="<?=$arNextBanner["IMAGE"]["src"]?>" class="lazy cat-slider-img" alt="">
							<?else:?>
								<img src="<?=$arNextBanner["IMAGE"]["src"]?>" class="lazy  cat-slider-img" alt="">
							<?endif;?>
						</a>
						<?if ($arNextBanner['MOB_IMAGE']){?>
						<a<?if(!empty($arNextBanner["MOB_LINK"])):?> href="<?=$arNextBanner["MOB_LINK"]?>"<?endif;?> class="cat-slider-mob">
							<?if(!empty($arParams["LAZY_LOAD_PICTURES"]) && $arParams["LAZY_LOAD_PICTURES"] == "Y"):?>
								<img src="<?=$templateFolder?>/images/lazy.jpg" data-lazy="<?=$arNextBanner["MOB_IMAGE"]["src"]?>" class="lazy cat-slider-img" alt="">
							<?else:?>
								<img src="<?=$arNextBanner["MOB_IMAGE"]["src"]?>" class="lazy  cat-slider-img" alt="">
							<?endif;?>
						</a>
						<?}?>
...
?>
		...
		<div id="catalog" class="productsListName" data-list-name="Каталог"  data-section-path="<?=$arResult['VARIABLES']['SECTION_CODE_PATH']?>">
			...
			<div id="ajaxSection">
				<?
				$view = strtolower(!empty($_REQUEST["VIEW"]) ? $_REQUEST["VIEW"] : (!empty($arParams['VIEW_MODE']) ? $arParams['VIEW_MODE'] : "squares"));

					setcookie("CATALOG_VIEW", $view, time() + 60 * 60 * 24 * 30 * 12 * 2);
...
<script>
_gcTracker.push(['view_category', { category_id: <?=$arCurSection['ID']?> }]);
</script>
