function shadowBlock(){
	if ($('#medi-openning-more-button').length){
		...
		$('#medi-openning-more-button').bind("click",function(){
            if (!$(this).data('status')) {
...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			...
			hrefFILTERbut = BX.findChildren(set_filter);

			if (result.FILTER_URL && hrefFILTER)
			{
				hrefFILTER[0].href = BX.util.htmlspecialcharsback(result.FILTER_URL);
				set_filter.href = BX.util.htmlspecialcharsback(result.FILTER_URL);
			...
			if (result.INSTANT_RELOAD && result.COMPONENT_CONTAINER_ID)
			{
				...
				$(".viewList .element a").each(function(){
					$class = 'squares';
...
$(window).on("load", function() {
	...
	if ($(".subexpander").length){
		$(".subexpander").on("click", function(){
			$this_id = $(this).data("id");
			if ($(this).children(".propExpand").hasClass("expanded"))
			{
				$(this).children(".propExpand").removeClass("expanded");
				$("#subexpand_"+$this_id).hide();
			}
			else {
				$(this).children(".propExpand").addClass("expanded");
				$("#subexpand_"+$this_id).show();
			}
		});
	}

	if ($(".sublevel li input:checked")){
		$(".sublevel li input:checked").parents(".sublevel").show();
	}
});
