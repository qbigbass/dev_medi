...
<div id="smartFilter">
	...
	<form name="<?=$arResult["FILTER_NAME"]."_form"?>" action="<?=$arResult["FORM_ACTION"]?>" method="GET" id="smartFilterForm">
		...
		foreach($arResult["ITEMS"] as $key => $arItem)
		{
			...
			if (in_array($arItem['ID'], $arParams['FILTER_HIDE_PROPS'])){
            ...
            if ($TYPE_GROUP_ACTIVE == "1" && $arItem['CODE'] == 'PRODUCT_TYPE') continue;
            ?>
			<div class="paramsBox">
				<div class="paramsBoxTitle">
					...
					switch ($arItem["DISPLAY_TYPE"])
					{
								...
								<li class="<?=($i > 5) ? "off" : ""?>">
									...
									<label for="<?=$ar["CONTROL_ID"]?>" class="<?=$class?>" data-role="label_<?=$ar["CONTROL_ID"]?>" onclick="smartFilter.keyup(BX('<?=CUtil::JSEscape($ar["CONTROL_ID"])?>')); BX.toggleClass(this, 'selected');">
										<?if (isset($ar["FILE"]) && !empty($ar["FILE"]["SRC"])):?>
											<span class="icon" title="<?=($ar['VALUE'])?>">
						...
						case "K"://RADIO_BUTTONS
							...
							<?if(count($arItem["VALUES"]) > 4):?>
								<li><a href="#" class="showALL theme-link-dashed"><?=GetMessage("CT_BCSF_FILTER_SHOW_ALL")?> <?=count($arItem["VALUES"])-4?></a></li>
							<?endif;?>
							</ul>
							<?
							break;
						...
						default://CHECKBOXES
							?>
							<?$i = 0;
                            if ($arItem['CODE'] == 'PRODUCT_TYPE_LINK'){
                                $lastlevel = ''; $TYPE_GROUP_ACTIVE = '1';?>
                                <ul class="checkbox">
                                    <?
                                    $cnt = 0;
                                    foreach($arItem["VALUES"] as $val => $ar):
                                        ...
                                        if (strpos($ar['VALUE'], '..')!== false)
                                        {
                                            $level = 2;

                                        }
                                        if ($lastlevel == '1' && $level == '1'){?></ul><?}
                                        if ($lastlevel == '2' && $level == '1'){?></ul><?}
                                        $i++;
                                        if ($level == '1') {
                                            $level_id = $ar['FACET_VALUE'];?>
                                                <li class="subexpander" data-id="<?=$level_id?>"><span class="type_name"><?=str_replace(".", '', $ar["VALUE"])?></span> <ins class="propExpand " ></ins></li><ul class="sublevel" id="subexpand_<?=$level_id?>">
                                            <li>
                                                <input
                                                        type="checkbox"
                                                        value="<?=$ar["HTML_VALUE"]?>"
                                                ...
                                                <label for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>">
                                                        <?=(substr($ar["VALUE"],-2) == 'ь' ? 'Вся' : 'Все')?>&nbsp;<span class="alltype_name"><?=str_replace(".", '', $ar["VALUE"])?></span>
                                                    <?if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):?>
                                                        <ins class="elCount" data-role="count_<?=$ar["CONTROL_ID"]?>">(<?=$ar["ELEMENT_COUNT"]?>)</ins>
                                                    <?endif;?>
                                                </label>
                                    ...
                                    <?/*if(count($arItem["VALUES"]) > 5  ):?>
                                        <li><a href="#" class="showALL theme-link-dashed" ><?=GetMessage("CT_BCSF_FILTER_SHOW_ALL")?> <?=count($arItem["VALUES"])-5?></a></li>
                                    <?endif;*/?>
                                </ul>
                            <?}else{
		...
		<div id="modef" style="display:none" <?//=(!isset($arResult["ELEMENT_COUNT"])) ? 'style="display:none"' : 'style="display: inline-block;"';?>>
