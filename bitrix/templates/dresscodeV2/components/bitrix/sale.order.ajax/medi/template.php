...
if (strlen($request->get('ORDER_ID')) > 0) {
    ...
    <form action="<?= POST_FORM_ACTION_URI ?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form"
        ...
        <?// Сохранение google client id в заказе
        ?>
        <input type="hidden" name="ORDER_PROP_21" value="<?= $_COOKIE['_ga']; ?>">
        <input type="hidden" name="ORDER_PROP_40" value="<?= $_COOKIE['_ym_uid']; ?>">
        <input type="hidden" name="ORDER_PROP_52" value="" id="soa-property-52">
        
        <? if ($USER->IsAuthorized()):
            if (array_intersect([20], $USER->GetUserGroupArray())) {
                ?>
                <input type="hidden" name="salon_name" id="salon_name_field"
                       value="<?= $_SESSION['SESS_AUTH']['FIRST_NAME']; ?> <?= $_SESSION['SESS_AUTH']['LAST_NAME']; ?> ">
            <?
            } ?>
        <?endif ?>
        ...
        <div id="bx-soa-order" class="row bx-<?= $arParams['TEMPLATE_THEME'] ?>" style="opacity: 0">
            ...
            <div class="col-sm-9 bx-soa">
                ...
                <!--	REGION BLOCK	-->
                <div id="bx-soa-region" data-visited="true" class="bx-soa-section bx-active">
                    ...
                    <div class="bx-soa-section-content container-fluid" style="display: none"></div>
                </div>
                ...
                <div id="bx-soa-orderSave">
                    <div class="checkbox">
                        ...
                        if ($arParams['USER_CONSENT'] === 'Y') {
                            $APPLICATION->IncludeComponent("bitrix:main.userconsent.request", "template", array(
                    ...
                    <a href="javascript:void(0)" id="go_to_order" style="margin: 10px 0"
                       class="pull-right btn btn-default btn-lg hidden-xs"
                       onsubmit="ym(30121774, 'reachGoal',   'SEND_ORDER'); return true;" data-save-button="true">
                        <?= $arParams['MESS_ORDER'] ?>
                    </a>
                </div>
