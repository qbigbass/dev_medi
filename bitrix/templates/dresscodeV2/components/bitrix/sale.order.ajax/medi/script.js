BX.saleOrderAjax = { // bad solution, actually, a singleton at the page
	...
	initDeferredControl: function()
	{
		...
		$zip = $("input[name='ORDER_PROP_6']").val();
		if ($zip.length == 10)
		{
			$.ajax({
				url: '/ajax/order/',
...
/*
					$suggest = $("input[name='ORDER_PROP_7']").suggestions({
					...
					$suggest.suggestions().setOptions({constraints: {
					  locations: { city: data },
					  deletable: false
					}});*/
				},
		...
		if ($("input[name='ORDER_PROP_3']").length) {
			var delFirstEight = {
			...
			// phone mask
			$("input[name='ORDER_PROP_3']").mask("79999999999", delFirstEight);
		...
		$(".bx-soa-item-td a").on("click", function(){

			var pos = $(this).parents('.bx-soa-item-tr').data('pos');
			var productList = 'Список Оформление заказа';
			var product_id = $(this).data('id');
			$.ajax({
			  url: "/ajax.php",
			  dataType: 'json',
			  data: {act: 'getItemData', product_id: product_id},
			  success: function(data){
		...
		/*if ($("input[name='ORDER_PROP_1']").length){

			$suggestions = $("input[name='ORDER_PROP_1']").suggestions({
				token: "3bf699bebe24b4c22576fce52726c0adec68917b",
				type: "NAME",
				minChars: 3,
			});
...
/*
		if ($("input[name='ORDER_PROP_2']").length){

			$suggestions = $("input[name='ORDER_PROP_2']").suggestions({
				token: "3bf699bebe24b4c22576fce52726c0adec68917b",
				type: "EMAIL",
				minChars: 2,
			});
		...
		$(".pull-right.btn").on("click", function(){
			sect_id = $(this).parents(".bx-soa-section").attr("id").replace('bx-soa-', '').replace('-hidden','');
			if (sect_id == 'region') $page = '/form-city';
			if (sect_id == 'delivery') $page = '/form-how-to-get';
			if (sect_id == 'paysystem') $page = '/form-how-to-pay';
			if (sect_id == 'properties') $page = '/form-name-telephone';
			ga('set', 'page', $page);
