                    ...
                    <? if ($arResult["isFormNote"] != "Y"): ?>
                        ...
                        <? if ($arResult["isFormDescription"] == "Y" || $arResult["isFormTitle"] == "Y" || $arResult["isFormImage"] == "Y"): ?>
                            <? if ($arResult["isFormTitle"]): ?>
                                <div class="webFormModalHeading"><?= $arResult["FORM_TITLE"] ?><a href="#"
                                        ...
                                        <div class="webFormItem" id="WEB_FORM_ITEM_<?= $FIELD_SID ?>">
                                            <div class="webFormItemCaption">
                                                <? if (!empty($arQuestion["IMAGE"])): ?>
                                                    ...
                                                    <img src="<?= $imageCaption["src"] ?>" class="webFormItemImage"
                                                         alt="<?= $arQuestion["CAPTION"] ?>">
                                                <? endif; ?>
                                                <div class="webFormItemLabel"><?= $arQuestion["CAPTION"] ?><? if ($arQuestion["REQUIRED"] == "Y"): ?>
                                                        <span class="webFormItemRequired">*</span><? endif; ?></div>
                                            </div>
                                                ...
                                                <? if ($arQuestion["STRUCTURE"]["0"]["FIELD_TYPE"] == "radio"): ?>
                                                    <? foreach ($arQuestion["STRUCTURE"] as $iq => $arNextStructureField): ?>
                                                        <div class="webFormItemFieldVariant">
                                                            <input type="radio" name="form_radio_<?= $FIELD_SID ?>"
                                                                   id="form_radio_<?= $FIELD_SID ?>_<?= $arNextStructureField["ID"] ?>"
                                                                   value="<?= $arNextStructureField["ID"] ?>">
                                        ...
                                        <div class="webFormItemField" data-required="Y">
                                            <input type="checkbox" class="personalInfoField" name="personalInfo"
                                                   value="Y"><label class="label-for"
                                                                    data-for="personalInfoField"><?= getMessage("PERSONAL_INFO_REQUIRED") ?>
                                                <span class="webFormItemRequired">*</span></label>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="tb">
                                <div class="tc">
                                    <input <?= (intval($arResult["F_RIGHT"]) < 10 ? "disabled=\"disabled\"" : ""); ?>
                                            ...
                                            class="sendWebFormDw"
                                            <? if ($arResult["arForm"]["ID"] == 2): ?>id="GTM_callback_form_header" onclick="ym(30121774,'reachGoal','callback_form')" <? endif; ?> />
                                    <input type="hidden" name="web_form_apply" value="Y"/>
                                </div>
                                <div class="tc"><input type="reset" value="<?= GetMessage("FORM_RESET"); ?>"/></div>
                            </div>
                            <p><span class="form-required starrequired">*</span>
                                - <?= GetMessage("FORM_REQUIRED_FIELDS") ?></p>
                        </div>
