...
$arResult['FORM_ID'] = '';
if ($arResult['PROPERTIES']['WEBFORM_LIST']['VALUE'] != ""
    && $arResult['PROPERTIES']['WEBFORM_LIST']['VALUE_SORT'] > 0) {
...
if (strpos($arResult['DETAIL_TEXT'], "#MAPSERVICE") > 0) {
    ...
    if (!empty($matches[1][0])) {
        ...
        if (!empty($servicesid) && $servicesid[0] != '') {
            $sFilter["UF_SERVICES"] = $servicesid;
        }
        ...
        //_c($servicesid);
        $resStore = CCatalogStore::GetList(array("SORT" => "ASC"), $sFilter, false, false,
            array("ID", "ADDRESS", "SITE_ID", "GPS_N", "GPS_S", "TITLE", "CODE", "SCHEDULE", "UF_*"));
        ...
        if (!empty($arStores)) {
            ...
            foreach ($arStores as &$arProperty):
                            ...
                            if (!empty($WORK_TIME[0])) {
                                ...
                                if ($work_hour_start[0] > 0) {
                                    ...
                                    if ($cnt == "1") {
                                        ...
                                        $when = 'в' . ($sweekday == 2 ? "о" : "") . ' ' . $day_of_week[$sweekday];
                                    }
                ...
                $obElement = CIBlockElement::GetList(
                    ...
                    [
                        "IBLOCK_ID" => 24,
                        "PROPERTY_STORE" => $arProperty['ID'],
                        'ACTIVE' => 'Y',
                        'ACTIVE_DATE' => 'Y'
                    ],
                    false,
                    false,
                    ["NAME", "PREVIEW_TEXT", "ACTIVE_TO"]
                );
                ...
                if ($arProperty["GPS_S"] != 0 && $arProperty["GPS_N"] != 0) {
                    ...
                    if ($arProperty['METRO']['NAME'] && !empty($arProperty['METRO']['SECTION']['ICON']['SRC'])) {
                        $sBalloonContent .= '<div class="map-balloon-metro map-balloon-tr"><img src="' .
                            $arProperty['METRO']['SECTION']['ICON']['SRC'] .
                            '" style="width:20px;height:20px;" title="' . $arProperty['METRO']['SECTION']['NAME'] .
                            '"><span>' . $arProperty['METRO']['NAME'] .
                            '</span></div> ';
                    }
                    ...
                    if (!empty($servicesid) && $servicesid[0] != '') {
                        ...
                        $sBalloonContent .= '<div class="map-balloon-footer "><a href="' . $arProperty['DETAIL_PAGE_URL'] . '" target=_blank class="btn-simple btn-micro select_button">Подробнее</a></div>';
                    }
        ...
        <script>
            ...
            function init() {
                ...
                if ($("#select_salon option:selected").length) {
