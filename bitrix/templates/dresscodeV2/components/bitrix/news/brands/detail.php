...
<?if(!empty($arResult)):?>
	...
	<?//$this->SetViewTarget("before_breadcrumb_container");?>
		<div class="blog-banner banner-wrap banner-bg">
			<div class="banner-animated fullscreen-banner banner-elem" style="background-image: url('<?=$arResult['ITEM']["RESIZE_BANNER_PICTURE"]["src"]?>');">
			...
			<?if(!empty($arResult['ITEM']["PROPERTY_BG_IMAGE_M_VALUE"])){?>
			<div class="banner-animated-small fullscreen-banner banner-elem" style="background-image: url('<?=$arResult['ITEM']["RESIZE_PREVIEW_PICTURE"]["src"]?>')">
...
<?if(!empty($ELEMENT_ID)):?>
	...
	<?if($countElements > 0):?>
		<div id="catalog"  data-list-name="Бренды" class="productsListName" <?if($countElements == 1):?>class="alone-item"<?endif;?>>
