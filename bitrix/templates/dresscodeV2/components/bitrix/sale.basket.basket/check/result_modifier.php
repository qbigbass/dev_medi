    ...
    foreach ($arResult['ITEMS'] as $k => $arItem) {
        ...
        if (!empty($arSalon['UF_SKLAD'])) {
            ...
            $rsStore = CCatalogStore::GetList(
                [],
                array('ACTIVE' => 'Y', 'XML_ID' => $arSalon['UF_SKLAD'], 'PRODUCT_ID' => $arItem['PRODUCT_ID']),
                false,
                false,
                array()
            );
            if ($arStore = $rsStore->Fetch()) {
                $arResult['ITEMS'][$k]['STORE'] = $arStore;
                $rsStoreAmount = CCatalogStoreProduct::GetList(
                    [],
                    array('STORE_ID' => $arStore['ID'], 'PRODUCT_ID' => $arItem['PRODUCT_ID']),
                    false,
                    false,
                    array()
                );
                if ($arStoreAmount = $rsStoreAmount->Fetch()) {
                    $arResult['ITEMS'][$k]['STORE_AMOUNT'] = $arStoreAmount;
                }
            }
        }
