...
if (!window.BX_YMapAddPlacemark)
{
	window.BX_YMapAddPlacemark = function(map, arPlacemark)
	{
		...
		var obPlacemark = new ymaps.Placemark(
			...
			props,
			{balloonCloseButton: true, iconLayout: 'default#image', iconImageHref: '//<?=SITE_SERVER_NAME.SITE_TEMPLATE_PATH?>/images/placemarker.png'}
		);
...
<?
	if (is_array($arResult['POSITION']['PLACEMARKS']) && ($cnt = count($arResult['POSITION']['PLACEMARKS']))):
		for($i = 0; $i < $cnt; $i++):
?>
	arObjects.PLACEMARKS[arObjects.PLACEMARKS.length] = BX_YMapAddPlacemark(map, <?echo CUtil::PhpToJsObject($arResult['POSITION']['PLACEMARKS'][$i])?>, {balloonCloseButton: true, iconLayout: 'default#image', iconImageHref: '//<?=SITE_SERVER_NAME.SITE_TEMPLATE_PATH?>/images/placemarker.png', iconImageSize: [28, 37],iconImageOffset: [-14, -37]});
<?
		endfor;
	endif;
