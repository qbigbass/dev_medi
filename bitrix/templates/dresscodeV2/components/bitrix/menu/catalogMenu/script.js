...
$(document).on("ready", function(event){
	...
	tools.sliceMainMenu = function(resize = true){
		...
		//other
		var visibleMenuWidth = $mainMenu.width() - 150;
		...
		if(viewport > 1024){
			...
			if($removedItems.length > 0){
				...
				//var $removedItemsLink = $("<li/>").addClass("removedItemsLink").append($("<a href='/stock/' class='menu_special'>Акции</a>"));
	...
	events.click.toggleMenu = function(event){
	  ...
	  //  $('body .overlay:first').fadeToggle(500);
		//block actions
		return event.preventDefault();
	...
	addSales = function(){
		...
		if ($href.includes('/personal/cart/order/') || $href.includes('/personal/cart/') ){
			target = 'target="_blank"';
		}
		$("#mainMenu").append('<span class="menu_stock"><a href="/stock/" class="menuLink" '+target+'>Акции</a></span>');
	}
	...
	events.click.authMenu = function (event) {
		$(".min_menu .topAuth").toggle();
		//block actions
		return event.preventDefault();
	}
	events.authMenuHide = function (event) {

		$(".min_menu .topAuth").hide();
	}
	...
	if($addsales != "0") 	addSales();

	//clear styles & attribures
	tools.clearAttributes();

	//binds
	$(document).on("click", "#main", events.authMenuHide );
	$(document).on("click", ".openAuth", events.click.authMenu);
