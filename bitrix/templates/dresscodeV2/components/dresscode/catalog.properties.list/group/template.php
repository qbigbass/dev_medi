    ...
    <div id="elementProperties">
        ...
        <table class="stats">
            ...
            <?foreach($arResult["PUBLIC_GROUPS"] as $arGroup):?>
                <?if(!empty($arGroup["NAME"]) && !empty($arGroup["PROPERTIES"])):?>
                    ...
                    <?foreach($arGroup["PROPERTIES"] as $nextProperty):
                        if (in_array($nextProperty['CODE'], ['PRODUCT_TYPE_GROUP', 'PRODUCT_TYPE_LINK'])) continue;?>
                        ...
                        <tr<?if($iteration % 2):?> class="gray"<?endif;?>>
                            ...
                            <td><?=strip_tags($nextProperty["DISPLAY_VALUE"])?></td>
            ...
            <?if(!empty($arResult["ANONYMOUS_PROPERTIES"])):?>
                <?$iteration = 1; //inc var for zebra table?>
                <tr class="cap">
                    <td colspan="2"><?//=GetMessage("CHARACTERISTICS")?></td>
                </tr>
                <?foreach($arResult["ANONYMOUS_PROPERTIES"] as $nextProperty):?>
                    <tr<?if($iteration % 2 ):?> class="gray"<?endif;?>>
                        <td class="name"><span><?=$nextProperty["NAME"]?></span><?if(!empty($nextProperty["HINT"])):?><span  class="question" title="<?=$nextProperty["HINT"]?>" data-description="<?=$nextProperty["HINT"]?>"></span><?endif;?></td>
                        <td><?=strip_tags($nextProperty["DISPLAY_VALUE"])?></td>
                        <?/*<td class="right">
