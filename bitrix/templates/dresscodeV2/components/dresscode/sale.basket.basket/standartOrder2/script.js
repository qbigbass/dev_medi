$(function(){
	...
	application.binding.click.deleteItem = function(event){
		...
		function dataProcessing(jsonData){
			...
			if(jsonData["status"] === true){
				...
				_rutarget.push({'event': 'removeFromCart', 'sku': jsonData['product']['ID'], 'qty': jsonData['product']['QUANTITY']});
	...
	application.binding.click.quantityMinus = function(event){
		...
		if(currentQuantity > measureRatio){
			...
			var _rutarget = window._rutarget || [];
			_rutarget.push({'event': 'updateInCart', 'sku': dl_product.data('id'), 'qty': $quantityField.val(), 'price': dl_product.data('product-price')});
