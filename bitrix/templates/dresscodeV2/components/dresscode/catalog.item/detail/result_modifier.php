...
if(!empty($arResult)){
	...
	if ($arResult['PARENT_PRODUCT']['IBLOCK_SECTION_ID'] > 0)
	{
		// определяем корневую категорию
		$arSects = [];
		$nav = CIBlockSection::GetNavChain(false, $arResult['PARENT_PRODUCT']['IBLOCK_SECTION_ID']);
	...
	$shoes_sect = '0';
	if ($arSects[0]['ID'] == 88 ||$arSects[1]['ID'] == 606)
	{
		$arResult['SHOES_CAT'] = "Y";
	}
	$arResult['SHOES_CAT'] = "N";
	$arResult['MKT_CAT'] = "N";
	// наличие на основных складах
	...
	if ($arResult["CATALOG_AVAILABLE"] == "N" && $shoes_sect == '1')
	{
		$mainStoreAmount = 0;
	}
	$arResult["mainStoreAmount"] = $mainStoreAmount;
