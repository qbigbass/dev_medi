...
<?if(!empty($arResult)):?>
	...
	?>
	<div class="item product sku itemCard" id="<?=$this->GetEditAreaId($arResult["ID"]);?>" data-product-iblock-id="<?=$arParams["IBLOCK_ID"]?>"  data-position = "<?=$arResult['POS_COUNT']?>"  data-from-cache="<?=$arResult["FROM_CACHE"]?>" data-convert-currency="<?=$arParams["CONVERT_CURRENCY"]?>" data-product-price="<?=$arResult['PRICE']['DISCOUNT_PRICE'];?>"  data-product-category="<?=implode("/",$secturl);?>" data-product-articul="<?=$arResult['PROPERTIES']['CML2_ARTICLE']['VALUE'];?>" data-product-brand="<?=$arResult['PARENT_PRODUCT']['BRAND'];?>"  data-currency-id="<?=$arParams["CURRENCY_ID"]?>" data-product-name="<?=($arResult['PARENT_PRODUCT']['NAME'] ? $arResult['PARENT_PRODUCT']['NAME'] :  $arResult['NAME']);?>"  data-product-id="<?=($arResult['PARENT_PRODUCT']['ID'] ? $arResult['PARENT_PRODUCT']['ID'] :  $arResult['ID']);?>" data-iblock-id="<?=$arResult["SKU_INFO"]["IBLOCK_ID"]?>" data-prop-id="<?=$arResult["SKU_INFO"]["SKU_PROPERTY_ID"]?>" data-product-width="<?=$arParams["PICTURE_WIDTH"]?>" data-product-height="<?=$arParams["PICTURE_HEIGHT"]?>" data-hide-measure="<?=$arParams["HIDE_MEASURES"]?>" data-currency="<?=$arResult["EXTRA_SETTINGS"]["CURRENCY"]?>" data-hide-not-available="<?=$arParams["HIDE_NOT_AVAILABLE"]?>" data-price-code="<?=implode("||", $arParams["PRODUCT_PRICE_CODE"])?>">
		    ...
		    <div class="productTable">
		    	<div class="productColImage">
					<a href="<?=$arResult["DETAIL_PAGE_URL"]?>" class="picture">
						...
						<span class="getFastView  <?=($arParams['LIST_TYPE']=='related_in_cart' ? 'fast' : '');?>" data-id="<?=$arResult["ID"]?>"><?=GetMessage("FAST_VIEW_PRODUCT_LABEL")?></span>
					...
					<?if(!empty($arResult["EXTRA_SETTINGS"])):?>
						...
						<?else:*/?>
							<a class="price"><?=CCurrencyLang::CurrencyFormat($arResult["PRICE"]["DISCOUNT_PRICE"], $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?>
								<?if ($arResult["PRICE"]["DISCOUNT_PRICE"]>0):?>
								<script>vViewedProdsPrice += <?=$arResult["PRICE"]["DISCOUNT_PRICE"]?>;</script>
								<?endif;?>
								<?if($arParams["HIDE_MEASURES"] != "Y" && !empty($arResult["EXTRA_SETTINGS"]["MEASURES"][$arResult["CATALOG_MEASURE"]]["SYMBOL_RUS"])):?>
									<span class="measure"> / <?=$arResult["EXTRA_SETTINGS"]["MEASURES"][$arResult["CATALOG_MEASURE"]]["SYMBOL_RUS"]?></span>
								<?endif;?>
                        ...
                        <?if($arResult['PROPERTIES']['COUNTRY_BRAND']['VALUE'] == 'Германия'){?><span class="flag-ge"></span><?}?>
						<?//endif;
						?>
							...
							<?//addCart button ?>
								...
								<?/*<a href="#" class="addCart disabled" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="<?=GetMessage("ADDCART_LABEL")?>" class="icon"><?=GetMessage("ADDCART_LABEL")?></a>*/?>
									...
									<?if ($arResult['DISPLAY_BUTTONS']['CART_BUTTON']):?>
										...
										<a href="#" class="fastBack fastBut label changeID " data-id="<?=$arResult["ID"]?>" <?if($arResult['SALON_AVAILABLE'] != "0" ||  $arResult['SALON_COUNT'] != "0" || $arResult["CATALOG_AVAILABLE"] == "Y"){?>style="display:none;"<?}?>style="display:none;"><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="<?=GetMessage("ORDER_NOTAVAIL_LABEL_ALT")?>" class="icon" ><?=GetMessage("ORDER_NOTAVAIL_LABEL")?></a>
									<?else:?>
