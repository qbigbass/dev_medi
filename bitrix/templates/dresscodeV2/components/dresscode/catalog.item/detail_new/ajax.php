...
<? if (CModule::IncludeModule("catalog") && CModule::IncludeModule("sale") && CModule::IncludeModule("dw.deluxe")) {
    ...
    $GLOBALS['max_price_code'] = 'CATALOG_PRICE_2';
    if (SITE_ID == 's2') {
        $GLOBALS['price_code'] = 'CATALOG_PRICE_6';
        $GLOBALS['max_price_code'] = 'CATALOG_PRICE_5';
    }
    ...
    if ($_REQUEST["act"] == "getFastDelivery") {
        ...
        if (!empty($_REQUEST["params"]) &&
            ...
            if (!defined('SITE_ID') && !empty($_REQUEST['msite_id'])) {
                define('SITE_ID', $_REQUEST['msite_id']);
            }
            ...
            if ($arOffermaxPrice = $obOfferPrice->GetNext()) {
                ...
                $arLastOffer['PRODUCT']['PRICE']['pricediff'] = $maxprice_diff;
            ...
            $shoes_sect = '0';
            if ($arSects[0]['ID'] == 88 || $arSects[0]['ID'] == 606) {
                $filter = array(
                    "ACTIVE" => "Y",
                    "!ID" => "52",
                    "PRODUCT_ID" => $arLastOffer["PRODUCT"]['ID'],
            ...
            if (in_array($arLastOffer['PRODUCT']['ID'], $excl_ids)) {
                $mainStoreAmount = 0;
            }
            ...
            if ($USER->IsAuthorized() && !empty(array_intersect([29], $USER->GetUserGroupArray()))) {
                $arLastOffer["PRODUCT"]['DISPLAY_BUTTONS']['SMP_BUTTON'] = true;
        ...
        $rsOffer = CIBlockElement::GetList(
            array(),
            array("ID" => intval($_REQUEST["id"]), "ACTIVE" => "Y", "ACTIVE_DATE" => ""), false, false,
            array(
                "ID",
                "NAME",
                "IBLOCK_ID"
