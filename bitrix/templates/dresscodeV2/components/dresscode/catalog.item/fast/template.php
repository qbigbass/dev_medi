...
<?if(!empty($arResult)):
	...
	<div id="appFastView" class="superitem item<?if(!empty($arResult["SKU_OFFERS"])):?> sku<?endif;?>" data-product-iblock-id="<?=$arParams["IBLOCK_ID"]?>" data-from-cache="<?=$arResult["FROM_CACHE"]?>" data-convert-currency="<?=$arParams["CONVERT_CURRENCY"]?>" data-currency-id="<?=$arParams["CURRENCY_ID"]?>" data-hide-not-available="<?=$arParams["HIDE_NOT_AVAILABLE"]?>"  data-product-name="<?=($arResult['PARENT_PRODUCT']['NAME'] ? $arResult['PARENT_PRODUCT']['NAME'] :  $arResult['NAME']);?>"  data-product-id="<?=($arResult['PARENT_PRODUCT']['ID'] ? $arResult['PARENT_PRODUCT']['ID'] :  $arResult['ID']);?>"  <?if(!empty($arResult["SKU_INFO"])):?> data-iblock-id="<?=$arResult["SKU_INFO"]["IBLOCK_ID"]?>" data-currency="<?=$arResult["EXTRA_SETTINGS"]["CURRENCY"]?>" data-prop-id="<?=$arResult["SKU_INFO"]["SKU_PROPERTY_ID"]?>"<?endif;?> data-product-width="<?=$arParams["PICTURE_WIDTH"]?>" data-product-height="<?=$arParams["PICTURE_HEIGHT"]?>" data-hide-measure="<?=$arParams["HIDE_MEASURES"]?>" data-cast-func="fastViewSku" data-change-prop="fast-view" data-more-pictures="Y" data-price-code="<?=implode("||", $arParams["PRODUCT_PRICE_CODE"])?>">
			...
			<div class="appFastViewColumnContainer">
				...
				<div class="appFastViewDescriptionColumn">
					<div class="appFastViewDescriptionColumnContainer">
						<div class="appFastViewProductHeading"><a href="<?=$arResult["DETAIL_PAGE_URL"]?>" class="appFastViewProductHeadingLink"><span class="appFastViewProductHeadingLinkLayout" data-section-path="<?=implode("/",$secturl);?>"><?=$arResult["NAME"]?></span></a></div>
													...
													<ul class="skuPropertyList">
														<?foreach ($arNextProp["VALUES"] as $xml_id => $arNextPropValue):?>
															<li class="skuPropertyValue<?if($arNextPropValue["DISABLED"] == "Y"):?> disabled<?elseif($arNextPropValue["SELECTED"] == "Y"):?> selected<?endif;?>" data-name="<?=$propName?>" data-value="<?=$arNextPropValue["VALUE"]?>">
																<a href="#" class="skuPropertyLink"  <?if (!empty($arNextPropValue["IMAGE"])): ?>title="<?=$arNextPropValue["DISPLAY_VALUE"] ?>"<?endif; ?>>
				...
				<div class="appFastViewInformationColumn">
					...
					<?if(!empty($arResult["PRICE"])):?>
						...
						<?else:?>
							<a class="price">
								...
								<?if ($arResult['PRICE']['PRICE']['PRICE'] < $max_price_mindiff):
										$price_diff = $max_price - $arResult['PRICE']['PRICE']['PRICE']; ?>
									<span class="oldPriceLabel"><?=GetMessage("FAST_VIEW_OLD_PRICE_LABEL")?><s class="discount"><?=CCurrencyLang::CurrencyFormat($max_price, $arResult["EXTRA_SETTINGS"]["CURRENCY"], true)?></s></span>
						...
						<?if($arResult["CATALOG_AVAILABLE"] != "Y"):?>
							...
							<?else:?>
								...
								<a href="#" class="fastBack  changeID" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="<?=GetMessage("ORDER_NOTAVAIL_LABEL_ALT")?>" class="icon"><?=GetMessage("ORDER_NOTAVAIL_LABEL")?></a>
							<?endif;?>
						<?else:?>
							<?// показ кнопки В корзину?>
							<?if ($arResult['DISPLAY_BUTTONS']['CART_BUTTON']):?>
							<a href="#" class="addCart <?=$arParams['IN_CART'] ? 'fast' : ''?>" data-id="<?=$arResult["ID"]?>"  id="GTM_add_cart_fast" ><img src="<?=SITE_TEMPLATE_PATH?>/images/incart.png" alt="" class="icon"><?=GetMessage("FAST_VIEW_ADDCART_LABEL")?></a>
							<?endif;?>
						<?endif;?>
					<?else:?>
					...
					<?endif;?>
					<?// показ кнопки В корзину включен ?>
					<?if ($arResult['DISPLAY_BUTTONS']['CART_BUTTON']):?>
