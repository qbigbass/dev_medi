...
<?if(!empty($arResult)):?>
	...
	?>
	<div class="itemRow item itemCard" id="<?=$this->GetEditAreaId($arResult["ID"]);?>" data-product-iblock-id="<?=$arParams["IBLOCK_ID"]?>"  data-position = "<?=$arParams['POS_COUNT']?>"  data-from-cache="<?=$arResult["FROM_CACHE"]?>" data-convert-currency="<?=$arParams["CONVERT_CURRENCY"]?>" data-currency-id="<?=$arParams["CURRENCY_ID"]?>"  data-product-price="<?=$arResult['PRICE']['DISCOUNT_PRICE'];?>"   data-product-category="<?=implode("/",$secturl);?>" data-product-articul="<?=$arResult['PROPERTIES']['CML2_ARTICLE']['VALUE'];?>" data-product-brand="<?=$arResult['PARENT_PRODUCT']['BRAND'];?>" data-hide-not-available="<?=$arParams["HIDE_NOT_AVAILABLE"]?>" data-product-name="<?=($arResult['PARENT_PRODUCT']['NAME'] ? $arResult['PARENT_PRODUCT']['NAME'] :  $arResult['NAME']);?>"  data-product-id="<?=($arResult['PARENT_PRODUCT']['ID'] ? $arResult['PARENT_PRODUCT']['ID'] :  $arResult['ID']);?>"  data-iblock-id="<?=$arResult["SKU_INFO"]["IBLOCK_ID"]?>" data-prop-id="<?=$arResult["SKU_INFO"]["SKU_PROPERTY_ID"]?>" data-product-width="<?=$arParams["PICTURE_WIDTH"]?>" data-product-height="<?=$arParams["PICTURE_HEIGHT"]?>" data-hide-measure="<?=$arParams["HIDE_MEASURES"]?>" data-currency="<?=$arResult["EXTRA_SETTINGS"]["CURRENCY"]?>" data-price-code="<?=implode("||", $arParams["PRODUCT_PRICE_CODE"])?>">
	<div class="column">
			...
			<?if($arResult["CATALOG_AVAILABLE"] != "Y"):?>
				...
				<?else:?>
					...
					<?if ($arResult['DISPLAY_BUTTONS']['CART_BUTTON']):?>
						<span class="fastBack fastBut changeID"  data-id="<?=$arResult["ID"]?>" ><?=GetMessage("FAST_BACK_PRODUCT_LABEL")?></span>
					<?else:?>
