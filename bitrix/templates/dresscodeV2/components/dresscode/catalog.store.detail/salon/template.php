...
<div class="salon-page">
	...
	<div class="salon-page-section">
			...
			<div class="si__content">
				...
				<?if(!empty($arResult["HOLIDAY_SHEDULE"])):?>
				  <div id="holiday_shedule_text">
						...
						<div class="holiday_shedule_block">
							<div class="corner_block"></div>
							<div class="close"></div>
				...
				<div class="si_shedule si_row">
					...
					<div class="si_shedule_content si_content">
						...
						<?if (!empty($arResult['UF_RR_MON']) && $arResult['HOLIDAY_SHEDULE']['PROPERTY_HIDE_RR_VALUE'] != 'Да'):?>
						<table>
							<tr>
		...
		<div class="salon-path salon-col-margin">
			<div class="sp_block">
				...
				<div class="sp_content"><p><?=htmlspecialchars_decode($arResult["UF_VECTOR"])?></p>
                <?if($arResult['TCMAP']){
                ?>
                    <span data-img="<?=$arResult['TCMAP']?>" class="theme-link-dashed get_medi_popup_Window" data-title="Схема расположения салона в ТЦ">Схема расположения салона в ТЦ</span><br/><br/>
                    <?
                }
                ?></div>
			</div>
	...
	<?$GLOBALS['stockFilter'] = ["PROPERTY_CITY_VALUE" => !empty($arResult['CITY']) ? $arResult['CITY'] : $GLOBALS['medi']['region_cities'][SITE_ID], "PROPERTY_HIDE"=>false];?>
	...
	<div class="salon-page-section">
		<?if (!empty($arResult['SERVICES']) && !empty($arResult['SERVICES_SLIDES'])){
...
<script>
	...
	function initMap()
	{
		...
		map = new ymaps.Map("map_salon", {
			center: [<?=$gpsN?>, <?=$gpsS?>],
			zoom: 16,
			controls: ['geolocationControl', 'fullscreenControl', 'zoomControl', 'routeButtonControl']
		});

        control = map.controls.get('routeButtonControl');

// Программно установим начальную точку маршрута.
        control.routePanel.state.set('to', '<?=$arResult['ADDRESS']?>');
...
<script>
fbq('track', 'ViewContent', {content_name: "Salon <?=$arResult['TITLE']?>"});

var _gcTracker=_gcTracker||[];
_gcTracker.push(['view_page', { name: 'Salon <?=$arResult['TITLE']?>'}]);
