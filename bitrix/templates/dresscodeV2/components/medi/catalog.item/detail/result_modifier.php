...
if(!empty($arResult)){
	...
	if (!empty($arResult['SKU_OFFERS'])){
		...
		$obOffer = CIBlockElement::GetList([], ["IBLOCK_ID" => $arResult['IBLOCK_ID'], "ID" => array_keys($arResult['ID'])], false, false, ["ID", "CATALOG_PRICE_2"]);
	...
	)
	{
		...
		if (!empty($action_prop_val))
		{
			...
			$obActions = CIBlockElement::GetList(
				['SORT'=>'ASC'],
				['IBLOCK_ID'=>6, '%PROPERTY_ACTION_TAG'=>$action_tags, 'ACTIVE'=>'Y', 'ACTIVE_DATE'=>'Y', "PROPERTY_CITY_VALUE" => $GLOBALS['medi']['region_cities'][SITE_ID]],
				false,
				false,
				['ID', 'IBLOCK_ID', 'NAME', 'DETAIL_PAGE_URL', 'PROPERTY_ACTION_TAG', 'DATE_ACTIVE_TO']
			);
...
/*
		...
		$getLoymaxPrice = $lmxapp->getLoymaxPrice($arResult['ID'], $arResult['IBLOCK_ID']);

		if (!empty($getLoymaxPrice) && $getLoymaxPrice['PRICE'] != $arResult['RESULT_PRICE']['DISCOUNT_PRICE']){

			$arResult['PRICE']['RESULT_PRICE']['BASE_PRICE'] = $getLoymaxPrice['BASE_PRICE'];
			$arResult['PRICE']['PRICE'] = $getLoymaxPrice['BASE_PRICE'];

			$arResult['PRICE']['RESULT_PRICE']['DISCOUNT_PRICE'] = $getLoymaxPrice['DISCOUNT'];

			$arResult['PRICE']['RESULT_PRICE']['DISCOUNT'] = $getLoymaxPrice['DISCOUNT'];

			$arResult['PRICE']['DISCOUNT_PRICE'] = $getLoymaxPrice['PRICE'];
			$arResult["PRICE"]["RESULT_PRICE"]["BASE_PRICE"] =  $arResult['RESULT_PRICE']['BASE_PRICE'];
			$arResult['MAX_PRICE'][$arResult['PRICE']['PRODUCT_ID']] = $arResult['PRICE']['PRICE'];

		}
		/*__($getLoymaxPrice);
		__($arResult["PRICE"]);
	__($arResult["MAX_PRICE"]);*/
}
