							...
							<?if(!empty($arResult["SKU_PROPERTIES"]) && $level = 1):?>
								...
								<?foreach ($arResult["SKU_PROPERTIES"] as $propName => $arNextProp):?>
									<?if(!empty($arNextProp["VALUES"])):?>
										...
										<?else:?>
											<div class="elementSkuProperty <?if (count($arNextProp["VALUES"]) == 2 && !in_array($propName, ['COLOR', 'SIZE']) ):?>fix_select<?endif;?>" data-name="<?=$propName?>" data-level="<?=$level++ ?>" data-highload="<?=$arNextProp["HIGHLOAD"]?>">
												<div class="elementSkuPropertyName"><?if ($propName=='SIZE') { echo "Выберите размер";}elseif ($propName=='COLOR') { echo "Выберите цвет";}else{echo preg_replace("/\[.*\]/", "", $arNextProp["NAME"]);} ?>:<?
											...
											?>
												...
												<ul class="elementSkuPropertyList">
													<?foreach ($arNextProp["VALUES"] as $xml_id => $arNextPropValue):?>
														<li class="elementSkuPropertyValue<?if($arNextPropValue["DISABLED"] == "Y"):?> disabled<?elseif($arNextPropValue["SELECTED"] == "Y"):?> selected<?endif;?>" data-name="<?=$propName?>" data-value="<?=$arNextPropValue["VALUE"]?>">
															<a href="#" class="elementSkuPropertyLink  <?=(!empty($arNextPropValue["IMAGE"]) ? 'withImg' : '');?>" <?if (!empty($arNextPropValue["IMAGE"])): ?>title="<?=$arNextPropValue["DISPLAY_VALUE"] ?>"<?endif; ?>>
												...
												<?else:?>
												    <select data-name="<?=$propName?>" class="sku-select">
                                                        <?foreach ($arNextProp["VALUES"] as $xml_id => $arNextPropValue):?>
                                                            <option class="" <?if($arNextPropValue["DISABLED"] == "Y"):?> disabled<?elseif($arNextPropValue["SELECTED"] == "Y"):?> selected<?endif;?> data-name="<?=$propName?>" data-value="<?=$arNextPropValue["VALUE"]?>">
				...
				*/?>
				...
				<div class="tabs-wrap">
					<div class="tabs-links">
						<div class="tab-link active">Описание</div>
						<div class="tab-link">Характеристики</div>

                        <?if($arParams["HIDE_AVAILABLE_TAB"] != "Y" && !$arResult['DONT_SHOW_REST']):?>
                        <div class="tab-link">Наличие в салонах</div>
                        <?endif;?>
						<div class="tab-link">Документы</div>
						<div class="tab-link"><?=GetMessage("REVIEW")?> (<?=count($arResult["REVIEWS"])?>)</div>	
					</div>
						...
						<div class="tab-content">
							...
							<?// NOTE: REVIEWS?>
								...
								<div id="catalogReviews">
									<h2 class="heading"><?if($arParams["SHOW_REVIEW_FORM"]):?><a href="#" class="reviewAddButton btn-simple btn-medium"><?=GetMessage("REVIEWS_ADD")?></a><?endif;?></h2>
									<ul id="reviews">
