	...
	var elementSelectSku = function(event, ignoreLoading){
		...
		if(!$_this.parent().hasClass("selected")){
			...
			function skuProcessingResult(jsonData){
				...
				fbq('track', 'ViewContent', {content_type: 'product', content_ids:[ jsonData[0]["PRODUCT"]["ID"]]});
				...
				if(jsonData[0]["PRODUCT"]["CATALOG_QUANTITY"] > 0){
					...
					if(jsonData[0]["PRODUCT"]["CAN_BUY"] != "Y"){
						...
						if(jsonData[0]["PRODUCT"]["CATALOG_SUBSCRIBE"] == "Y" && jsonData[0]["PRODUCT"]["PRICE"]["DISCOUNT_PRICE"]){
							...
							$changeCart.html("Под заказ").prepend(
								$("<img/>").addClass("icon").attr("src", TEMPLATE_PATH + "/images/incart.png"))
								.attr("href", "#").removeClass("addCart").addClass("fastBack label").removeClass("disabled").show();
							}
							else {
								$changeCart.hide();
							}
				...
				//write discount
				$productPriceBlock.prepend($productPriceContainer);
				//write price container with price value
				$productChangePrice.html($productPriceBlock);
