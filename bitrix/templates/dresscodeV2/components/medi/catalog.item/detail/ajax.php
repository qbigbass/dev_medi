	...
	elseif($_REQUEST["act"] == "selectSku"){
		if(!empty($_REQUEST["params"]) &&
		   ...
		   !empty($_REQUEST["props"])
			...
			$price_id = SITE_ID == 's2' ? 6 : 1;
			$max_price_id = SITE_ID == 's2' ? 5 : 2;
			...
			{
				$maxprice_diff = $arOffermaxPrice['CATALOG_PRICE_'.$GLOBALS['price_code']] + 100;
				if ($arOffermaxPrice['CATALOG_PRICE_'.$GLOBALS['price_code']] > $maxprice_diff)
				{
					$arLastOffer['PRODUCT']['PRICE']['RESULT_PRICE']['DISCOUNT_PRICE'] = CCurrencyLang::CurrencyFormat($arOffermaxPrice[$GLOBALS['price_code']] - $arOffermaxPrice[$GLOBALS['price_code']], "RUB", true);
					$arLastOffer['PRODUCT']['PRICE']['RESULT_PRICE']['DISCOUNT'] = ($arOffermaxPrice[$GLOBALS['price_code']] - $arOffermaxPrice[$GLOBALS['max_price_code']]);
					$arLastOffer['PRODUCT']['PRICE']['RESULT_PRICE']['DISCOUNT_PRINT'] = CCurrencyLang::CurrencyFormat(($arOffermaxPrice[$GLOBALS['price_code']] - $arOffermaxPrice[$GLOBALS['price_code']]), "RUB", true);
					$arLastOffer['PRODUCT']['PRICE']['PRICE']['BASE_PRICE'] = CCurrencyLang::CurrencyFormat($arOffermaxPrice[$GLOBALS['max_price_code']], "RUB", true);
					$arLastOffer['PRODUCT']['PRICE']['RESULT_PRICE']['BASE_PRICE'] = CCurrencyLang::CurrencyFormat($arOffermaxPrice[$GLOBALS['max_price_code']], "RUB", true);
				}
			}
			...
			if ($arLastOffer["PRODUCT"]['SALON_COUNT'] > 0) {
				...
				$snDeclension = new Declension('салона', 'салонов', 'салонов');
				$avail_salons = $sDeclension->get($arLastOffer["PRODUCT"]['SALON_COUNT']);
				$avail_salons_now = $snDeclension->get($arLastOffer["PRODUCT"]['SALON_COUNT']);
				$arLastOffer["PRODUCT"]['SALON_COUNT_STR'] = '<div class="ff-medium row h3">Наличие в салонах</div>
				Товар <a href="#stores" class="available_link link" >в наличии в '.$arLastOffer["PRODUCT"]['SALON_COUNT'].'&nbsp;'.$avail_salons.'</a>';
				$arLastOffer["PRODUCT"]['SALONS_AVAIL_NOW'] = 'Из&nbsp;'.$arLastOffer["PRODUCT"]['SALON_COUNT'].'&nbsp;'.$avail_salons_now. ' medi сегодня';
...
function picture_separate_array_push($pictureID, $arPushImage = array()){
	if(!empty($pictureID)){
		...
		$arPushImage = array();
		$arPushImage["SMALL_IMAGE"] = array_change_key_case(CFile::ResizeImageGet($pictureID, array("width" => 60, "height" => 60), BX_RESIZE_IMAGE_PROPORTIONAL, false), CASE_UPPER);
...
function getLastOffer($arLastFilter, $arProps, $productID, $opCurrency, $arPrices = array()){
	...
	if(!$rsLastOffer->SelectedRowsCount()){
		...
		if($obReturnOffer = $rsLastOffer->GetNextElement()){
			...
			if ($productFilelds["PRICE"]["RESULT_PRICE"]["DISCOUNT_PRICE"] > 0)
