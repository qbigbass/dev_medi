<?if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true) die();?>
   ...
   ?>
		   ...
		   <h1><?=GetMessage("PRODUCT_FOR_SEARCH")." &laquo;".htmlspecialcharsbx($arResult['REQUEST']["~QUERY"])."&raquo;"?></h1>
...
<div id="catalogColumn">
   <div class="leftColumn">
	   ...
	   <?if(count($arResult["SEARCH"]) > 1):
		   ...
		   ?><?
           global $preFilter;
           $preFilter["PROPERTY_REGION_VALUE"] = $GLOBALS['medi']['region_cities'][SITE_ID];
           $preFilter['ID'] = array_keys($arResult["ELEMENTS"]);$preFilter['ACTIVE'] = "Y";
           ?>
		   <?$APPLICATION->IncludeComponent(
			   ...
			   array(
				   ...
				   "ELEMENTS_ID" => array_keys($arResult["ELEMENTS"]),
				   "CACHE_TYPE" => "N",
				   "CACHE_TIME" => "36000000",
				   "CACHE_GROUPS" => "Y",
				   "SAVE_IN_SESSION" => "N",
   ...
   <div class="rightColumn">
			   ...
			   <?
				   ...
				   if(empty($arTemplateSettings["TEMPLATE_USE_AUTO_SAVE_PRICE"]) || $arTemplateSettings["TEMPLATE_USE_AUTO_SAVE_PRICE"] == "N"){
					   $arSortFields["PRICE_ASC"] = array(
						   "ORDER"=> "ASC",
						   "CODE" => "CATALOG_PRICE_".(SITE_ID == 's2' ? "5" : "1"),
						   "NAME" => GetMessage("CATALOG_SORT_FIELD_PRICE_ASC")
					   );
	   ...
	   foreach($arResult["SEARCH"] as $k=>$arItem):
		...
		endforeach;?>
	   <?if (!empty($arResult["SEARCH"])):?>
		   <?$APPLICATION->SetTitle(GetMessage("PRODUCT_FOR_SEARCH")."&laquo;".htmlspecialcharsbx($arResult['REQUEST']["ORIGINAL_QUERY"]."&raquo;"));?>
		   ...
		   ?>
		   <div id="catalog" <?if(count($arResult["SEARCH"]) == 1):?>class="alone-item"<?endif;?>  class="productsListName" data-list-name="Результаты поиска" >
			   <?$arrFilter["FACET_OPTIONS"] = array();?>

			   <?$arrFilter["PROPERTY_REGION_VALUE"] = $GLOBALS['medi']['region_cities'][SITE_ID];
			   $arrFilter['ID'] = array_keys($arResult["ELEMENTS"])?>
			   <?reset($arTemplates); ?>
			   <?$APPLICATION->IncludeComponent(
