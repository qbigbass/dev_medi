...
if ($this->startResultCache())
{
	...
	if ($isProductExistSKU)
	{
		$res = CIBlockElement::GetList(
			...
			false,
			false,
			array('ID', 'IBLOCK_ID', "PROPERTY_NO_RESERV_BUTTON")
		);
		if ($productInfo = $res->Fetch())
		{
			...
			$arParams["NO_RESERV_BUTTON"]  = $productInfo['PROPERTY_NO_RESERV_BUTTON_VALUE'];
		}
	...
	else
	{
		...
		$select = array_merge(
			array("ID", "ACTIVE", "PRODUCT_AMOUNT", "TITLE", "TYPE", "CODE", "SITE_ID", "UF_*"),
			$arParams["FIELDS"],
		...
		while ($prop = $rsProps->GetNext())
		{
			...
			// если в салоне нет, и отключен заказ

			if ($amount == 0 && $prop['UF_ESHOP_ORDERS'] == 0)
				contine;
			...
			$storeName = $prop["UF_STORE_PUBLIC_NAME"];// . " (" . $prop["ADDRESS"] . ")";

			if ($prop["ADDRESS"] != ''){
				$storeAddress =  preg_replace("/[0-9]{6},/", "", $prop["ADDRESS"]);
			}
			else {
				$storeAddress = "";
			}
			...
			$metro  = unserialize($prop['UF_METRO']);
	        if (!empty($metro)) {
			...
			$storeInformation = array(
				...
				"CODE" => $prop['CODE'],
				"BOOKING" => ($arParams['NO_RESERV_BUTTON'] == 'Да' ? 0 :$prop['UF_BOOKING']),
				"UF_ESHOP_ORDERS" => $prop['UF_ESHOP_ORDERS'],
				"UF_RR_MON" => $prop["UF_RR_MON"],
				"UF_RR_TUE" => $prop["UF_RR_TUE"],
				"UF_RR_WED" => $prop["UF_RR_WED"],
				"UF_RR_THU" => $prop["UF_RR_THU"],
				"UF_RR_FRI" => $prop["UF_RR_FRI"],
				"UF_RR_SAT" => $prop["UF_RR_SAT"],
				"UF_RR_SUN" => $prop["UF_RR_SUN"], 
				"METRO" => $metroSalons,
			);
