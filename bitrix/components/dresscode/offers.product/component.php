	...
	if($this->StartResultCache($arParams["CACHE_TIME"], $cacheID)){
		...
		if(!empty($arParams["PROP_VALUE"])){
			...
			foreach ($arParams["PROP_VALUE"] as $ipr => $PROP_ID){
				...
				$arFilter = Array("IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"], "IBLOCK_ID" => $arParams["IBLOCK_ID"], "PROPERTY_".$arParams["PROP_NAME"] => $PROP_ID, "ACTIVE_DATE" => "Y", "ACTIVE" => "Y", "PROPERTY_REGION_VALUE"=> $GLOBALS['medi']['region_cities'][SITE_ID]);
