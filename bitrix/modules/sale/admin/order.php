...
else
{
	if($saleModulePermissions == "W" || ($saleModulePermissions >= 'P' && !empty($allowedStatusesUpdate) ))
	{
		...
		if (array_intersect([9,11], $USER->GetUserGroupArray())){
			$rsCount = 1;
			$arSitesShopID = 's1';
		}
		elseif (array_intersect([18,19], $USER->GetUserGroupArray())){
			$rsCount = 1;
			$arSitesShopID = 's2';
		}
		else {
			$arSitesShopID = $arSitesShop[0]["ID"];
		}
		if($rsCount == 1)
		{
			$siteLID = "&SITE_ID=".$arSitesShopID;
		}
		else
		{
