    ...
    public function ArrayToExcel( $fileFullName, $fileTitle, $arData, $arProfile, $page ){
        if( !empty( $fileFullName ) && is_array( $arData ) ) {
            ...
            try{
                ...
                if( $objPHPExcel ){
                    $objPHPExcel->getProperties()->setCreator( "acrit.exportpro" )
                    ->setLastModifiedBy( "acrit.exportpro" )
                    ->setTitle( $fileTitle )
...
class CAcritExportproMarketCategories{
    ...
    private function GetLocalFilePath( $yandexCategoryServerPath ){
        ...
        $yandexCategoryLocalPath = $_SERVER["DOCUMENT_ROOT"]."/upload/acrit.exportpro/".$yandexCategoryFileName;
