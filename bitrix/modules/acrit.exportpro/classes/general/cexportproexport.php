...
class CAcritExportproExport{
    ...
    public function __construct( $profileID ){
        ...
        $this->baseDir = $this->documentRoot."/acrit.exportpro/";
        ...
        if( strlen( strstr( self::$fileExport, $this->baseDir ) ) > 0 ){
            self::$fileExport = str_replace( $this->baseDir, $this->documentRoot."/upload/acrit.exportpro/", self::$fileExport );
    ...
    public function Export( $type = "", $cronpage = 0, $processId = 0 ){
        ...
        if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.exportpro/export_{$this->profile["ID"]}_run.unlock" )
            || ( $bBaseCron && file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.exportpro/export_{$this->profile["ID"]}_run.lock" ) && !$cronpage ) ){
            if( !$bBaseCron ){
                $this->Unlock();
            }
            return false;
        ...
        if( !$end ){
                    ...
                    for( $i = 0; $i < $steps2; $i++ ){
                        for( $j = 0; $j < $tCnt; $j++ ){
                            ...
                            $threadsId = $threads->newThread(
                                $this->documentRoot."/bitrix/modules/acrit.exportpro/tools/cronrun_proc.php",
                                array(
            ...
            if( !$allPages && $allPages != -1 && !$cronrun ){
                ...
                if( $this->profile["USE_COMPRESS"] == "Y" ){
                    ...
                    elseif( file_exists( $this->documentRoot."/upload/acrit.exportpro/".$this->originalNamePath ) ){
                        $zipSavePath = $this->documentRoot."/upload/acrit.exportpro/".$this->originalNamePath;
                    }
    ...
    private function Unlock(){
        ...
        AcritExportproSession::DeleteSession( $this->profile["ID"], $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.exportpro/" );
    }
