...
function unlock_export(){
    ...
    if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.exportpro/export_{$profileID}_run.lock" ) ){
        ...
        file_put_contents( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.exportpro/export_{$profileID}_run.unlock", "" );
    ...
    if( !isset( $_REQUEST["ID"] ) ){
        ...
        <?if( strlen( $types[$PROFILE["TYPE"]]["PORTAL_VALIDATOR"] ) > 0 ){?>
            <div style="float: left;">
                <input type="text" size="30" name="PROFILE[SETUP][URL_DATA_FILE]" id="URL_DATA_FILE" value="/acrit.exportpro/<?=$profileCode?>.<?if( $PROFILE["TYPE"] == "advantshop" ):?>csv<?else:?>xml<?endif;?>"/>
