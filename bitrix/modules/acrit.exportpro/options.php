...
if( $POST_RIGHT >= "R" ){
	...
	if( ( $REQUEST_METHOD == "POST" ) && ( strlen( $Update.$Apply.$RestoreDefaults ) > 0 ) && ( $POST_RIGHT == "W" ) && check_bitrix_sessid() ){
        if( isset( $_REQUEST["ACRITMENU_GROUPNAME"] ) && ( strlen( trim( $_REQUEST["ACRITMENU_GROUPNAME"] ) ) > 0 ) ){
            COption::SetOptionString( "acrit.exportpro", "acritmenu_groupname", trim( $_REQUEST["ACRITMENU_GROUPNAME"] ) );
        }
		$Update = $Update.$Apply;
		ob_start();
	...
	<form method="post" action="<?=$APPLICATION->GetCurPage()?>?mid=<?=urlencode( $module_id )?>&amp;lang=<?=LANGUAGE_ID?>">
        ...
        <tr>
            <td colspan="2" class="adm-detail-content-cell" align="center">
                <input type="text" name="ACRITMENU_GROUPNAME" value="<?=COption::GetOptionString( "acrit.exportpro", "acritmenu_groupname" );?>"/>
            </td>
        </tr>
        <tr>
            ...
            <td colspan="2" class="adm-detail-content-cell" align="center">
                <a class="adm-btn adm-btn-save" onclick="javascript:UpdateBitrixCloudMonitoring( '<?=GetMessage( "SC_SET_BITRIX_CLOUD_MONITORING_EMAIL" );?>' );">
                    <?if( $bExistBitrixCloudMonitoring ){
                            echo GetMessage( "ACRIT_EXPORTPRO_SET_BITRIX_CLOUD_MONITORING_BUTTON_OFF" );
                    }
                    else{
                        echo GetMessage( "ACRIT_EXPORTPRO_SET_BITRIX_CLOUD_MONITORING_BUTTON_ON" );
                    }?>
                </a>
            </td>
        </tr>
