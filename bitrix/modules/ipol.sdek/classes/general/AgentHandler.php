...
class AgentHandler extends abstractGeneral
{
    ...
    */
    ...
    {
        $agents = array(
            Tools::getMessage('AGENT_UPDATELIST')  => array("updateList", self::$MODULE_ID), //обновление листов
            Tools::getMessage('AGENT_ORDERSTATES') => array("orderStates",self::$MODULE_ID,option::get('orderStatusesAgentRollback') * 60), //обновление статусов заказов
            Tools::getMessage('AGENT_ORDERCHECKS') => array("getSendedOrdersState",self::$MODULE_ID,1800) // проверяем cdek_number у отправленных заявок
        );
