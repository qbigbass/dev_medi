...
class sdekOption extends sdekHelper
{
            ...
            foreach ($reqParams as $parCode => $parVal) {
                ...
                switch ($parCode) {
                    case 'currency':
                        break;
                    case "AS"      :
                        foreach ($parVal as $code => $noThing)
                            if (array_key_exists($code, $adServises))
                                $paramsSrt .= $adServises[$code]['NAME'] . " (" . $code . "), ";
                        $paramsSrt = substr($paramsSrt, 0, strlen($paramsSrt) - 2) . "<br>";
                        break;
                    case "GABS"    :
                        $paramsSrt .= $parVal['D_L'] . "x" . $parVal['D_W'] . "x" . $parVal['D_H'] . " " . GetMessage("IPOLSDEK_cm") . " " . $parVal['W'] . " " . GetMessage('IPOLSDEK_kg') . "<br>";
                        break;
                    case "service" :
                        $paramsSrt .= $tarifs[$parVal]['NAME'] . "<br>";
                        break;
                    case "courierTimeBeg":
                        $paramsSrt .= GetMessage("IPOLSDEK_JS_SOD_courierTime") . ": " . $parVal . " - " . $reqParams["courierTimeEnd"] . "<br>";
                        break;
                    case "courierTimeEnd":
                        break;
                    case "packs"   :
                    ...
                    case 'toPay'   :
                    case 'deliveryP' :
                        $paramsSrt .= sdekExport::formatCurrency(array('TO' => $reqParams['currency'], 'SUM' => $parVal, 'FORMAT' => true)) . "<br>";
                        break;
                    case 'NDSDelivery' :
                    case 'NDSGoods'       :
                        $paramsSrt .= GetMessage('IPOLSDEK_NDS_' . $parVal) . "<br>";
                        break;
                    case 'departure':
                    case 'location' :
                        $city = sqlSdekCity::getBySId($parVal);
    ...
    static function placeFAQ($code)
    {
        ?>
        <a class="ipol_header"
           onclick="$(this).next().toggle(); return false;"><?= GetMessage('IPOLSDEK_FAQ_' . $code . '_TITLE') ?></a>
    ...
    static function placeStatuses($option)
    {
        if (self::canShipment()) {
            ...
            <td></td>
            <td>
                <div class='IPOLSDEK_sepTable'><?= GetMessage('IPOLSDEK_STT_order') ?></div>
                <div class='IPOLSDEK_sepTable'><?= GetMessage('IPOLSDEK_STT_shipment') ?></div>
            </td></tr><?php
    ...
    { // Сбрасываем счетчик заявок в опциях
    ...
    { // Убираем информацию об обновлении
    ...
    { // запрос на синхронизацию из опций
        $arReturn = false;
        ...
        if (!$arReturn) {
            ...
            switch ($us['result']) {
                case 'error'   :
                    $arReturn['text'] = GetMessage("IPOLSDEK_SYNCTY_ERR_HAPPENING") . " " . $us['error'];
                    break;
                case 'end'     :
                    $arReturn['text'] = (array_key_exists('full', $params) && $params['full']) ? GetMessage('IPOLSDEK_UPDT_DONE') . date("d.m.Y H:i:s", filemtime($_SERVER["DOCUMENT_ROOT"] . "/bitrix/js/" . self::$MODULE_ID . "/list.json")) : GetMessage('IPOLSDEK_SYNCTY_LBL_SCOD');
                    break;
                case 'country' :
                    $arReturn['text'] = GetMessage('IPOLSDEK_SYNCTY_CNTRCTDONE') . GetMessage('IPOLSDEK_SYNCTY_' . $us['country']);
                    break;
                default        :
                    $arReturn['text'] = GetMessage('IPOLSDEK_SYNCTY_LBL_PROCESS') . " " . $us['done'] . "/" . $us['total'];
                    break;
            }
        }
    ...
    { // вызов обновления списка городов, самовывозов и услуг
        if (!self::updateList())
    ...
    static function getOrderState($params)
    {
        ...
        if ($result['code'] != 200)
            ...
            $arOrder = array(array(
                ...
                'Reason' => (string)$xml->Order->Reason['Description'],
            ));
    ...
    private static function setOrderStates($arOrders)
    {
        ...
        foreach ($arOrders as $orderMess) {
            if (array_key_exists($orderMess['State'], $arStateCorr)) {// описан ли статус
                ...
                if (intval($orderMess['State']) == 12 &&
                    trim($orderMess['Reason']) == mb_convert_encoding('Частичная доставка', 'utf-8',
                        'windows-1251')) {
                    $orderMess['State'] = 4;
    ...
    { // список стран для синхронизации
    ...
    static function handleImport($params)
    {
        ...
        switch ($params['mode']) {
            case 'setSync':
                $sync = self::cityUpdater($_REQUEST['timeOut']);
    ...
    {// проверка наличия заявки для заказа / отгрузки
        if (!self::isAdmin('R'))
            return false;
