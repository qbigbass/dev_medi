...
if($deliveryId > 0)
{
	...
	if (strpos($service['CLASS_NAME'], '\Sale\Handlers\Delivery\RussianpostProfile') !== false && $service['CONFIG']['MAIN']['SERVICE_TYPE'] == 1)
	{
		if($order_id > 0)
		{
			...
			$ajaxData['message'] = '<div class="crm-entity-widget-content-block-inner" id="russianpost_info_block"><a href="javascript:void(0)" onclick="event.preventDefault(); openMap(\'' . $guid_id . '\', ' . $price_basket . ',' . intval($weight) . ',[' . $strZip . '], \'' . $location . '\');">'.Loc::getMessage('SALE_DLV_RUSSIANPOST_POST_LINK').'</a>';
			...
			$ajaxData['message'] .= '<input type="hidden" id="russianpost_crm_data" name="russianpost_crm_data" value="Y"></div>';
		}
