...
class Action extends TradingService\Reference\Action\DataAction
{
	...
	protected function makePricesChunk($productIds, $chunkSize)
	{
		...
		foreach (array_chunk($productIds, $chunkSize) as $productIdsChunk)
		{
			...
			$prices = $this->applyPricesSku($prices, $skuMap);
	...
	protected function applyPricesSku($prices, $skuMap)
	{
	...
	protected function buildOffers($prices)
	{
		...
		foreach ($prices as $price)
		{
			...
			if (!empty($price['BASE_PRICE']) && $price['BASE_PRICE'] > $price['PRICE'])
			{
				$discount = $price['BASE_PRICE'] - $price['PRICE'];
				$discountPercent = 100 * floor($discount) / ceil($price['BASE_PRICE']);

				if ($discount >= 1 && $discountPercent >= 5)
				{
					$offer['price']['discountBase'] = $price['BASE_PRICE'];
				}
			}
	...
	{
		$request = new TradingService\Marketplace\Api\SendPrices\Request();
		$options = $this->provider->getOptions();
