...
class DeliveryOption extends TradingService\Reference\Options\Fieldset
{
		...
		return [
			...
			'INVERTIBLE' => [
				...
				'DEPEND' => [
					...
					'ID' => [
						'RULE' => 'ANY',
						'VALUE' => $this->filterCourierInvertible($environment, array_column($deliveryEnum, 'ID')),
					],
					'OUTLET_TYPE' => [
						'RULE' => 'ANY',
	...
	protected function getDigitalFields(TradingEntity\Reference\Environment $environment, $siteId)
	{
		...
		return
			$this->getDigitalCommonFields($digitalAdapters, $environment, $siteId)
			+ $this->getDigitalSettingFields($digitalAdapters, $environment, $siteId);
	}

	protected function getDigitalCommonFields(array $digitalAdapters, TradingEntity\Reference\Environment $environment, $siteId)
	{
		return [
			'AUTO_FINISH' => [
				'TYPE' => 'boolean',
	...
	protected function getDigitalSettingFields(array $digitalAdapters, TradingEntity\Reference\Environment $environment, $siteId)
	{
		$result = [];
	...
	protected function getDeliveriesWithoutPeriod(TradingEntity\Reference\Environment $environment, array $deliveryIds)
	{
		return $environment->getDelivery()->filterServicesWithoutPeriod($deliveryIds);
	}

	protected function filterCourierInvertible(TradingEntity\Reference\Environment $environment, array $deliveryIds)
	{
		$result = [];
		$courierRegistry = $environment->getCourierRegistry();

		foreach ($deliveryIds as $deliveryId)
		{
			$courier = $courierRegistry->resolveCourier($deliveryId);

			if ($courier instanceof TradingEntity\Reference\CourierInvertible)
			{
				$result[] = $deliveryId;
			}
		}
