...
class Action extends TradingService\Marketplace\Action\Cart\Action
{
	...
	protected function collectDelivery()
	{
		...
		foreach ($deliveryIds as $deliveryId)
		{
			...
			if ($this->isDeliveryInvertible($deliveryId, $deliveryOption))
			{
	...
	protected function isDeliveryInvertible($deliveryId, TradingService\MarketplaceDbs\Options\DeliveryOption $deliveryOption = null)
	{
		if ($deliveryOption !== null)
		{
			return $deliveryOption->isInvertible();
		}
		...
		if ($type === TradingService\MarketplaceDbs\Delivery::TYPE_PICKUP)
		{
			$outlet = $this->environment->getOutletRegistry()->resolveOutlet($deliveryId);

			$result = ($outlet instanceof TradingEntity\Reference\OutletInvertible);
		}
		else if ($type === TradingService\MarketplaceDbs\Delivery::TYPE_DELIVERY)
		{
	...
	)
	{
		...
		if ($calculationResult->getDeliveryType() === TradingService\MarketplaceDbs\Delivery::TYPE_DELIVERY)
		{
			...
			$useIntervals = (
				$deliveryOption === null
				|| $deliveryOption->getIntervalFormat() === TradingService\MarketplaceDbs\Options\DeliveryOption::INTERVAL_FORMAT_TIME
			);

			if (!empty($dateIntervals) && $useIntervals)
			{
