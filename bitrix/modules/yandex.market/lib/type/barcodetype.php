    ...
    public function validate($value, array $context = [], Market\Export\Xml\Reference\Node $node = null, Market\Result\XmlNode $nodeResult = null)
    {
        ...
        else
        {
            ...
            if (!isset(static::$availableLengthMap[$length]))
            {
				...
				Market\Data\TextString::getPosition($valueDigits, '2') === 0
				|| Market\Data\TextString::getPosition($valueDigits, '02') === 0
			)
			{
				$result = false;
				...
				if ($nodeResult)
				{
					$nodeResult->registerError(self::getMessage('ERROR_SHOP_INTERNAL'));
				}
			}
        }
