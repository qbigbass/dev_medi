<?
$ModuleID = "up.boxberrydelivery_spb";
require_once($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/main/include/prolog_admin_before.php');
require_once($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/up.boxberrydelivery_spb/include.php');
$ModuleID = "up.boxberrydelivery_spb";
...
$sTableID = "tbl_boxberry_spb_export";
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W"){
	$bbObject = new CBoxberryParsel_spb;
	foreach($arID as $ID){
...
$allDeliverys = \Bitrix\Sale\Delivery\Services\Manager::getActiveList();
	foreach ($allDeliverys as $profile){
		if (strpos($profile['CODE'],'boxberry_spb')!==false && strpos($profile['CODE'],'KD')!==false){
			$boxberry_profiles[] = $profile['ID'];
			$boxberry_profiles_kd[] = $profile['ID'];

		}elseif (strpos($profile['CODE'],'boxberry_spb')!==false && strpos($profile['CODE'],'PVZ')!==false){
			$boxberry_profiles[] = $profile['ID'];
...
$rsData->NavStart();
$lAdmin->NavText($rsData->GetNavPrint(GetMessage("BB_PAGING_TITLE_SPB")));
...
while($arRes = $rsData->NavNext(true, "f_"))
{
		...
		$arBbOrder = CBoxberryOrder_spb::GetByOrderId($f_ID);
        ...
        $row->AddViewField("ACCOUNT_NUMBER",  $arRes["ACCOUNT_NUMBER"]);
...
$APPLICATION->SetTitle(GetMessage('BB_PAGE_TITLE_SPB'));
...
<tr>
  ...
  <td nowrap>
    <?=GetMessage("BB_FROM_ALT");?><input type="text" name="find_account_number_from" size="20" value="<?echo htmlspecialchars($find_account_number_from)?>">
    <?=GetMessage("BB_TO_ALT");?><input type="text" name="find_account_number_to" size="20" value="<?echo htmlspecialchars($find_account_number_to)?>">
  </td>
</tr>
<tr>
