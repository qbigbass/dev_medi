...
class CDeliveryBoxberry_spb
{
    ...
    protected static $module_id='up.boxberrydelivery_spb';

    public static function Init()
    {
		CModule::IncludeModule(self::$module_id);
		if (!CBoxberry_spb::init_api()) return array("ERROR" => GetMessage('WRONG_API_CONNECT'));
		self::$widget = CBoxberry_spb::method_exec('GetKeyIntegration');
		self::$widget['settings_spb'] = CBoxberry_spb::method_exec('WidgetSettings',NULL,TRUE);
		$GLOBALS['bxb_settings_spb'] = $settings;
		$GLOBALS['key_spb'] = self::$widget['key_spb'];
		$GLOBALS['widget_settings_spb'] = self::$widget['settings_spb'];
		return array(
          "SID" => "boxberry_spb",
		  ...
		  "HANDLER" => __FILE__,
          "DBGETSETTINGS" => array("CDeliveryBoxberry_spb", "GetSettings_spb"),
          "DBSETSETTINGS" => array("CDeliveryBoxberry_spb", "SetSettings_spb"),
          "GETCONFIG" => array("CDeliveryBoxberry_spb", "GetConfig_spb"),

          "COMPABILITY" => array("CDeliveryBoxberry_spb", "Compability"),
          "CALCULATOR" => array("CDeliveryBoxberry_spb", "Calculate"),
	...
	public static function WidgetInit_spb() {
	global $APPLICATION;
		if (strpos($APPLICATION->GetCurPage(), 'bitrix/admin') === false || !ADMIN_SECTION)
		{
			$GLOBALS['APPLICATION']->IncludeComponent("bberry_spb:boxberry.widget", "", array(),false);
		}
	}
	public static function GetConfig_spb()
	{
	   $arConfig = array(
			"CONFIG" => array(
				"default" => array(),
    ...
    public static function SetSettings_spb($arSettings)
    {
    ...
    public static function GetSettings_spb($strSettings)
    {
	...
	{
        if ($possible_boxberry_points = CBoxberry_spb::method_exec('ListPoints', array('CityCode='.$city_code.'&prepaid=1'))){
	...
	{
	    $regionBitrixName = str_replace('ё', 'е', self::$region_bitrix_name);
	    $cityBitrixName = str_replace('ё', 'е', self::$city_bitrix_name);

		$boxberry_list = CBoxberry_spb::method_exec('ListCities');
		$i=0;
			...
			foreach($boxberry_list as $boxberry_cities){
				...
				if (strpos($regionBitrixName, $region_name) !== false){
			...
			foreach($boxberry_city as $cities){
				if (strpos($cityBitrixName, $cities['Name'])  !== false){
					return $cities["Code"];
				}
			}
		return false;
    ...
    public static function GetZipKD ($zip_code){
        $possible_zip = CBoxberry_spb::method_exec('ZipCheck', array('Zip='.$zip_code));
	...
	public static function Calculate($profile, $arConfig, $arOrder, $STEP, $TEMP = false)
    {
		...
		$bxb_custom_link = COption::GetOptionString('up.boxberrydelivery_spb', 'BB_CUSTOM_LINK');
		...
		if ($profile == 'PVZ'){
				...
				$price_delivery = CBoxberry_spb::method_exec('DeliveryCosts',$arrParams, TRUE);
	...
	function orderCreate($id,$arOrder){
		if (COption::GetOptionString(self::$module_id, 'BB_LOG') == 'Y'){
				CBoxberry_spb::$log->save($id);
				CBoxberry_spb::$log->save($arOrder);
				CBoxberry_spb::$log->save($_SESSION);
		}
		if (!function_exists('findParentBXB2'))
		{
			function findParentBXB2($profiles){
		...
		$parent = array_filter ($allDeliverys, 'findParentBXB2');
		...
		{
			$result = CBoxberry_spb::MakePropsArray($arOrder);
			$arFields = array(
				'ORDER_ID' 			=> $id,
			...
			CBoxberryOrder_spb::Add($arFields);
		}
		return true;
	}
...
AddEventHandler("sale", "OnSaleComponentOrderOneStepDelivery", array('CDeliveryBoxberry_spb', 'WidgetInit_spb'));
AddEventHandler("sale", "onSaleDeliveryHandlersBuildList", array('CDeliveryBoxberry_spb', 'Init'));
AddEventHandler("sale", "OnSaleComponentOrderOneStepComplete", array('CDeliveryBoxberry_spb', 'orderCreate'));
AddEventHandler("sale", "OnOrderUpdate", array('CDeliveryBoxberry_spb', 'orderCreate'));
?>
