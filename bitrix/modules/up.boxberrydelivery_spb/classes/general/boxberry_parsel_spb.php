<?
class CBoxberryParsel_spb
{
	static $module_id = "up.boxberrydelivery_spb";

    function ParselCreate($ORDER_ID)
    {
    	if (!CBoxberry_spb::init_api()) array("ERROR" => GetMessage('WRONG_API_CONNECT'));
		...
		if(!$order = CBoxberry_spb::GetFullOrderData($ORDER_ID)) return array('ERROR' => 'Order not found.');
        if(!$arProps = CBoxberry_spb::MakePropsArray($order)) return array('ERROR' => 'Wrong module settings.');
		$order_id = (COption::GetOptionString('up.boxberrydelivery_spb', 'BB_ACCOUNT_NUMBER') == 'Y' ?  $order['ACCOUNT_NUMBER'] :  $order['ID']);
		...
		$bxbOptions['bb_paid_person_jur'] = COption::GetOptionString('up.boxberrydelivery_spb', 'BB_PAID_PERSON_JUR');
		...
		if (strtoupper(LANG_CHARSET) != 'UTF-8'){
			$SDATA = CBoxberry_spb::mb_conv_array($SDATA);
		}

		$data = CBoxberry_spb::method_exec_post('ParselCreate',$SDATA);
