...
class Log_spb{
	...
	function __construct(){
		self::$LOG_FILE = 'boxberrydelivery_spb.log';
		self::$LOG_DIRECTORY = $_SERVER['DOCUMENT_ROOT']. '/bitrix/cache/log/';
		if (COption::GetOptionString(CBoxberry_spb::$module_id, 'BB_LOG') == 'Y'){
...
class CBoxberry_spb
{
    static $module_id = "up.boxberrydelivery_spb";
	static $err = null;
	static $curl = null;
	...
	function init_api()
	{
		...
		if (self::$allow_url_fopen != "1"){
			if (extension_loaded('curl')){
				self::$curl = new curl_bxb_api_spb();
				return true;
			}else{
				return false;
			}
    ...
    function GetFullOrderData($ORDER_ID)
    {
    	if(intval($ORDER_ID) <= 0) return false;
			...
			$bxb_order_info = CBoxberryOrder_spb::GetByOrderId($ORDER_ID);
    ...
    function MakePropsArray($order)
    {
    	...
    	$arReturn = array();
			...
			$arReturn['VID'] = (($order['DELIVERY_ID'] == 'boxberry_spb:PVZ' || $order['DELIVERY_ID'] == 'boxberry_spb:PVZ_COD') ? 1 : 2);
			$arUserFields = $USER->GetByID($order['USER_ID'])->Fetch();
			$arOptFields = array(
				'BB_FIO'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_FIO'),
				'BB_CONTACT_PERSON'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_CONTACT_PERSON'),
				'BB_PHONE'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_PHONE'),
				'BB_EMAIL'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_EMAIL'),
				'BB_ZIP'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_ZIP'),
				'BB_LOCATION'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_LOCATION'),
				'BB_ADDRESS'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_ADDRESS'),
				'BB_JUR_ADDRESS'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_JUR_ADDRESS'),
				'BB_COMPANY_NAME'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_COMPANY_NAME'),
				'BB_INN'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_INN'),
				'BB_KPP'=> COption::GetOptionString('up.boxberrydelivery_spb', 'BB_KPP'),
			);
	...
	function changeAddress ($ORDER_ID=NULL, $address=NULL){
		if (!empty($address)){
			$dbProps = CSaleOrderPropsValue::GetOrderProps($ORDER_ID);
			$address_prop_bb = COption::GetOptionString('up.boxberrydelivery_spb', 'BB_ADDRESS');
			$location_prop_bb = COption::GetOptionString('up.boxberrydelivery_spb', 'BB_LOCATION');
			if (strtoupper(LANG_CHARSET) != 'UTF-8'){
	...
	function updatePVZ($id_pvz, $ORDER_ID=NULL, $address=NULL){
		...
		CBoxberryOrder_spb::Update($ORDER_ID,$arFields);
