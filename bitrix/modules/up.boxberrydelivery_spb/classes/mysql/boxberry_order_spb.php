<?
class CBoxberryOrder_spb {

	function err_mess()
	{
		$module_id = "up.boxberrydelivery_spb";
		include($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/".$module_id."/install/version.php");
		return "<br>Module: ".$module_id." (".$arModuleVersion["VERSION"].")<br>Class: CBoxberryOrder_spb<br>File: ".__FILE__;
	}
    ...
    function Add($arFields)
    {
		...
		$arInsert = $DB->PrepareInsert("b_boxberry_order_spb", $arFields);
		$strSql = "INSERT INTO b_boxberry_order_spb(".$arInsert[0].") VALUES(".$arInsert[1].")";
    ...
    function Update($ORDER_ID, $arFields)
    {
        ...
        global $DB;
        $strUpdate = $DB->PrepareUpdate("b_boxberry_order_spb", $arFields);
        $strSql = "UPDATE b_boxberry_order_spb SET ".$strUpdate." WHERE ORDER_ID='".$DB->ForSql(intval($ORDER_ID))."'";
    ...
    function GetByOrderId($ORDER_ID)
    {
        ...
        global $DB;
        $strSql = "SELECT * FROM b_boxberry_order_spb WHERE ORDER_ID='".$DB->ForSql(intval($ORDER_ID))."' ORDER BY `ID` DESC";
        $res = $DB->Query($strSql, true);
		if (!empty($res)){
			$arRes = $res->Fetch();
			return $arRes;
    ...
    function Delete($ORDER_ID)
    {
		...
		global $DB;
		$err_mess = (self::err_mess())."<br>Function: Add<br>Line: ";
		$strSql = "DELETE FROM b_boxberry_order_spb WHERE ORDER_ID='".$ORDER_ID."'";
