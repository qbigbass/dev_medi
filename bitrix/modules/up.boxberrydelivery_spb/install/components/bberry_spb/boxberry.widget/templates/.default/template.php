...
$bxbOptions['address'] = COption::GetOptionString('up.boxberrydelivery_spb', 'BB_ADDRESS');
$bxbOptions['bb_custom_link'] = COption::GetOptionString('up.boxberrydelivery_spb', 'BB_CUSTOM_LINK');
$bxbOptions['bb_paid_person_ph'] = COption::GetOptionString('up.boxberrydelivery_spb', 'BB_PAID_PERSON_PH');
...
{
   if (strpos($arOrder['DELIVERY_ID'], 'boxberry_spb') === false ){ $adminBoxberry=false; }
	...
	function getLink()
	{
		$.ajax({
			...
			success: function(return_data){
				$('#'+bb_custom_link).html("");
				$('#'+bb_custom_link).append('<a href="#" onclick="boxberry_spb.checkLocation(1);boxberry_spb.open('+  makeWidgetString(return_data) +');return false;" ><?=GetMessage("BB_CUSTOM_LINK");?></a>');
			}
		});
	}
