...
class NCLNameCaseRu extends NCLNameCaseCore
{
     ...
     * @var array
     */
    private $ovo = ['ово', 'аго', 'яго', 'ирь'];
    /**
     * Окончания имен/фамилий, который не склоняются
     * @var array
     */
    private $ih = ['их', 'ых', 'ко', 'уа'/*Бенуа, Франсуа*/];
    /**
...
//Проверяем действительно ли отчество
    ...
    protected function GenderByFirstName(NCLNameCaseWord $word)
    {
        ...
        if ($this->in($this->Last(2),
            ['он', 'ов', 'ав', 'ам', 'ол', 'ан', 'рд', 'мп', 'по'/*Филиппо*/, 'до'/*Леонардо*/, 'др', 'рт'])) {
            $man += 0.3;
        }
        if ($this->in($this->Last(1), $this->consonant)) {
            $man += 0.01;
        ...
        if ($this->in($this->Last(3),
            ['лья', 'вва', 'ока', 'ука', 'ита', 'эль'/*Рафаэль, Габриэль*/, 'реа'/*Андреа*/])) {
            $man += 0.2;
        }
...
//если смахивает на отчество
        ...
        if ($this->Last(1) == 'ь') {
             ...
             * Просто исключения
             */
            elseif ($this->inNames($namepart, ['Лазарь', 'Игорь', 'Любовь'])) {
                $first += 10;
            } /**
         ...
         */
        elseif ($this->in($this->Last(1), $this->consonant . 'ь') and $this->in($this->Last(2, 1),
                $this->consonant . 'ь')) {
            /**
...
//Исключения
        ...
        if ($this->Last(3) == 'ина') {
             ...
             * Исключения
             */
            elseif ($this->inNames($namepart, [
                'Мальвина', 'Антонина', 'Альбина', 'Агриппина', 'Фаина', 'Карина', 'Марина', 'Валентина', 'Калина',
                'Аделина', 'Алина', 'Ангелина', 'Галина', 'Каролина', 'Павлина', 'Полина', 'Элина', 'Мина', 'Нина',
                'Дина'
            ])) {
                $first += 10;
            } /**
         ...
         */
        if ($this->in($this->Last(2), [
            'ов', 'ин', 'ев', 'ёв', 'ый', 'ын', 'ой', 'ук', 'як', 'ца', 'ун', 'ок', 'ая', 'ёк', 'ив', 'ус', 'ак', 'яр',
            'уз', 'ах', 'ай'
        ])) {
            $second += 0.4;
        }
        ...
        if ($this->in($this->Last(3), [
            'ова', 'ева', 'ёва', 'ына', 'шен', 'мей', 'вка', 'шир', 'бан', 'чий', 'кий', 'бей', 'чан', 'ган', 'ким',
            'кан', 'мар', 'лис'
        ])) {
            $second += 0.4;
        }
