...
class SiteLocation
{
     ...
     */
    public static function getCodesBySiteId(string $siteId = null): array
    {
        if (!isset($siteId)) {
            return [];
        }
     ...
     * @throws Main\SystemException
     */
    public static function isCodeAllowsToSite(string $code, string $siteId = null): bool
    {
        if (!isset($siteId)) {
            return true;
        }

        $siteLocationsCodes = self::getCodesBySiteId($siteId);

        return empty($siteLocationsCodes) || in_array($code, $siteLocationsCodes);
    }
}