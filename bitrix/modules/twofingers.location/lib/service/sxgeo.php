...
class SxGeo
{
    ...
    protected static $url = 'https://sypexgeo.net/files/SxGeoCity_utf8.zip';
    ...
    public function __construct($db_file = 'SxGeo.dat', $type = SXGEO_FILE)
    {
        ...
        if (substr($header, 0, 3) != 'SxG') {
            throw new SystemException("Can't open {$db_file}\n");
        }
        ...
        if ($info['b_idx_len'] * $info['m_idx_len'] * $info['range'] * $info['db_items'] * $info['time'] * $info['id_len'] == 0) {
            throw new SystemException("Wrong file format {$db_file}\n");
        }
     ...
     * @return string
     */
    protected static function getDbFileName(): string
    {
        return $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/twofingers.location/include/SxGeoCity.dat';
    }
     ...
     */
    public static function getInstance(): SxGeo
    {
        if (!isset(self::$instance)) {

            if (!file_exists(self::getDbFileName())) {
                throw new SystemException('db file not exists (' . self::getDbFileName() . ')');
            }

            self::$instance = new self(
                self::getDbFileName(),
                Options::isSxGeoMemory() ? SXGEO_MEMORY : SXGEO_FILE
            );
        }
     ...
     */
    public static function updateGeoBase(): string
    {
        set_time_limit(600);
        ...
        if (Options::isSxGeoProxyEnabled() && strlen(Options::getSxGeoProxyName())) {
            curl_setopt($ch, CURLOPT_PROXY, Options::getSxGeoProxyName() . ':' . (Options::getSxGeoProxyPort() ?: 80));
            curl_setopt($ch, CURLOPT_PROXYTYPE, Options::getSxGeoProxyType());
            if (Options::getSxGeoProxyPass()) {
                curl_setopt($ch, CURLOPT_PROXYUSERPWD, Options::getSxGeoProxyPass());
            }
        }

        if (!curl_exec($ch)) {
            throw new SystemException(Loc::getMessage('TFL__UPDATE_ERROR_DOWNLOAD', ['#error#' => curl_error($ch)]));
        }
        ...
        if ($code != 200) {
            ...
            throw new SystemException(Loc::getMessage('TFL__UPDATE_ERROR_DOWNLOAD', ['#error#' => $code]));
        }
     ...
     * @throws ArgumentOutOfRangeException
     */
    public static function getLastUpdateGMT(): string
    {
    ...
    {
        $gmt = self::getLastUpdateGMT();

        return \DateTime::createFromFormat('D, d M Y H:i:s \G\M\T', $gmt);
    }
