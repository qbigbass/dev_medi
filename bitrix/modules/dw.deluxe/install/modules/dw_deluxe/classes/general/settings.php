...
class DwSettings{
			...
			if(!empty($arThemes)){
				...
				foreach($arThemes as $inx => $nextTheme){
					...
					if(!file_exists($themesDirectory.$nextTheme."/style.css")){
						...
						if(!empty($arThemesByLevel)){
							foreach($arThemesByLevel as $ix => $nextThemeByLevel){
								if(file_exists($themesDirectory.$nextTheme."/".$nextThemeByLevel."/style.css")){
									//get theme colors
									$arThemeColors = self::getThemeColorsByDirectory($themesDirectory.$nextTheme."/".$nextThemeByLevel);
									//push theme colors
									if(!empty($arThemeColors)){
										self::$themeColors[$nextTheme]["VARIANTS"][$nextThemeByLevel] = $arThemeColors;
									}
									//theme level 2 found
						...
						//get theme colors
						$arThemeColors = self::getThemeColorsByDirectory($themesDirectory.$nextTheme);
						//push theme colors
						if(!empty($arThemeColors)){
							self::$themeColors[$nextTheme] = $arThemeColors;
						}
						//theme level 1 found
    ...
    public function getThemeColorsByDirectory($themeDirectory){

	   	//check transmitted
    	if(!empty($themeDirectory)){

			//check style.css
			if(file_exists($themeDirectory."/style.css")){

				//get style.css
				$styleCode = file_get_contents($themeDirectory."/style.css");
				...
				if(!empty($styleCode)){
					...
					//get base color
					preg_match('/Base color: (\#[a-zA-Z0-9]{6})/im', $styleCode, $arColors["base"]);

					//get base color hover
					preg_match('/Base color hover: (\#[a-zA-Z0-9]{6})/im', $styleCode, $arColors["hover"]);

					//get base color focus
					preg_match('/Base color focus: (\#[a-zA-Z0-9]{6})/im', $styleCode, $arColors["focus"]);

					//check empty
					foreach($arColors as $currentColor){
						if(empty($currentColor[1])){
							return false;
						}
					}
					...
					//result
					return array(
						"BASE_COLOR" => $arColors["base"][1],
						"BASE_COLOR_HOVER" => $arColors["hover"][1],
						"BASE_COLOR_FOCUS" => $arColors["focus"][1]
					);
	...
	public function getCurrentSettings(){
		...
		$arSettingsName = array(
			...
			"TEMPLATE_USE_AUTO_AVAILABLE_PRODUCTS",
			"TEMPLATE_MASKED_INPUT_CUSTOM_FORMAT",
	...
	public function saveSettings($requestData, $requestFiles){
		...
		if(!empty($requestData)){
			...
			foreach($requestData as $settingName => $settingValue){
				...
				//specialchars
				$settingValue = htmlspecialcharsbx($settingValue);
			...
			//utf8 control
			if(!defined("BX_UTF")){
				$settingsWrite = \Bitrix\Main\Text\Encoding::convertEncoding($settingsWrite, "UTF-8", LANG_CHARSET);
			}
	...
	public static function copyDirFiles($src, $dst){
		...
		while(false !== ($file = readdir($dir))){
			...
			if(($file != ".") && ($file != "..")){
				...
				if(is_dir($src . "/" . $file)){
					//recursive
					self::copyDirFiles($src . "/" . $file, $dst . "/" . $file);
				}
	...
	public static function isBot(){
		...
		//check
		return (str_replace($botAgents, "", $_SERVER["HTTP_USER_AGENT"]) != $_SERVER["HTTP_USER_AGENT"]);
	}
