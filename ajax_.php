			...
			if(!empty($_GET["product_id"])){
				...
				if ($productInfo = $rsBaseProduct->GetNext())
				{
				    if ($productInfo['PROPERTY_CML2_LINK_ID'] > 0)
				    {
				        ...
				        $rsBaseProduct2 = CIBlockElement::GetList([], $arFilter, false, false, ['ID', 'NAME', 'DETAIL_PAGE_URL', 'CATALOG_PRICE_1', 'PROPERTY_ATT_BRAND.NAME', 'PROPERTY_CML2_ARTICLE']);
				        if ($productBrand = $rsBaseProduct2->GetNext())
				        {
				            ...
				            $goodPrice = $productBrand['CATALOG_PRICE_1'];
		...
		elseif($_GET["act"] == "selectSku"){
			if(!empty($_GET["params"]) &&
			   ...
			   !empty($_GET["props"])
				...
				$obOfferPrice = CIBlockElement::GetList([], ["IBLOCK_ID" => $arLastOffer["PRODUCT"]['IBLOCK_ID'], "ID" => $arLastOffer["PRODUCT"]['ID']], false, false, ["CATALOG_PRICE_2", "CATALOG_PRICE_1"]);
				if ($arOffermaxPrice = $obOfferPrice->GetNext())
				{
					$maxprice_diff = $arOffermaxPrice['CATALOG_PRICE_1'] + 100;
					if ($arOffermaxPrice['CATALOG_PRICE_2'] > $maxprice_diff)
					{
						$arLastOffer['PRODUCT']['PRICE']['RESULT_PRICE']['DISCOUNT_PRICE'] = CCurrencyLang::CurrencyFormat($arOffermaxPrice['CATALOG_PRICE_2'] - $arOffermaxPrice['CATALOG_PRICE_1'], "RUB", true);
						$arLastOffer['PRODUCT']['PRICE']['RESULT_PRICE']['DISCOUNT'] = ($arOffermaxPrice['CATALOG_PRICE_2'] - $arOffermaxPrice['CATALOG_PRICE_1']);
						$arLastOffer['PRODUCT']['PRICE']['RESULT_PRICE']['DISCOUNT_PRINT'] = CCurrencyLang::CurrencyFormat(($arOffermaxPrice['CATALOG_PRICE_2'] - $arOffermaxPrice['CATALOG_PRICE_1']), "RUB", true);
						$arLastOffer['PRODUCT']['PRICE']['PRICE']['BASE_PRICE'] = CCurrencyLang::CurrencyFormat($arOffermaxPrice['CATALOG_PRICE_2'], "RUB", true);
						$arLastOffer['PRODUCT']['PRICE']['RESULT_PRICE']['BASE_PRICE'] = CCurrencyLang::CurrencyFormat($arOffermaxPrice['CATALOG_PRICE_2'], "RUB", true);
					}
				}
			...
			//single
				...
				//push basket window component
					...
					if(!empty($skuParentProduct)){
					    ...
					    $rsBaseProduct = CIBlockElement::GetList([], $arFilter, false, false, ['ID', 'NAME', 'PROPERTY_ATT_BRAND.NAME',  'DETAIL_PAGE_URL', 'CATALOG_PRICE_1']);
					    if ($productBrand = $rsBaseProduct->GetNext())
					    {
					        $addedProduct['BRAND'] = $productBrand['PROPERTY_ATT_BRAND_NAME'];
					        $addedProduct['PRICE'] = $productBrand['CATALOG_PRICE_1'];
					    ...
					    $rsBaseProduct = CIBlockElement::GetList([], $arFilter, false, false, ['ID', 'PROPERTY_ATT_BRAND.NAME', 'PROPERTY_CML2_ARTICLE', 'CATALOG_PRICE_1', 'DETAIL_PAGE_URL', 'NAME']);
						    if ($productBrand = $rsBaseProduct->GetNext())
					    {
					        $addedProduct['BRAND'] = $productBrand['PROPERTY_ATT_BRAND_NAME'];
				...
				if(!empty($_GET["phone"]) && !empty($_GET["id"])){
					...
					if(CModule::IncludeModule("iblock") && CModule::IncludeModule("sale") && CModule::IncludeModule("catalog") && CModule::IncludeModule("dw.deluxe") && CModule::IncludeModule("form")){
						...
						if(!empty($arElement)){
							...
							$obElm = CIBlockElement::GetList([], ["ID" => $arElement["ID"]], false, false, ["IBLOCK_ID", "NAME", "DETAIL_PAGE_URL", "CATALOG_PRICE_2"] );
						    if ($arElm = $obElm->GetNext())
						    {
