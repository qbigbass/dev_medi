...
// Получим все предложения с активной опцией "Активная размерная характеристики СПБ" для всех ранее полученных товаров
$arrOffersProducts = CCatalogSKU::getOffersList(
    $arrProducts,
    17,
    [
        "ACTIVE" => "Y",
        "PROPERTY_SELECTED_SIZE_CHARACT_SPB" => $valueEnumSelectedSize
    ],
    [
        "ID",
        "IBLOCK_ID",
...
if (!empty($arrOffersProducts)) {
    ...
    // Оставим только те ТП, которых нет на складах в г. Санкт-Петербург
                ...
                foreach ($offersListProduct as $productId => $arrOffers) {
                    foreach ($arrOffers as $index => $dataSku) {
                        ...
                        if ($arrStoreOffer[$xmlId] > 0) {
                            ...
                            // Снимаем чекбокс в св-ве "Активная размерная характеристика СПБ" у текущего ТП
                            $propertyValues = [
                                "SELECTED_SIZE_CHARACT_SPB" => ''
                            ];
