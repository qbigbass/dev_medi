				...
				<div class="form_row ">
		            <div class="form_cell">
						...
						<label class="webFormItemCaption" for="<?=$field_name;?>"><?=$arQuestion['CAPTION']?><?=($arQuestion['REQUIRED']=='Y' ? '<span class="webFormItemRequired">*</span>' : '');?></label>
		    				<input type="tel" placeholder="+7 (___) ___-__-__" id="<?=$field_name;?>"  <?=($arQuestion['REQUIRED']=='Y' ? 'required' : '');?> class="<?=($FIELD_SID == 'PHONE' ? 'phonemask' : '');?> <?=$FIELD_SID?>_input"  name="<?=$field_name;?>" value="<?=(isset($_REQUEST[$field_name]) ? htmlspecialchars($_REQUEST[$field_name]) : ($_SESSION['lmx']['phone'] ? $_SESSION['lmx']['phone'] : ''));?>" maxlength="18">
		    				<p class="error_<?=$field_name;?>"><span style="display:none">Поле  не заполнено</span></p>
		    			</div>
		            </div>
		        </div>
				...
				<div class="form_column">
			        ...
			        <div class="form_row ">
			            <div class="form_cell">
			                ...
			                <select name="form_text_<?=$arQuestion['STRUCTURE'][0]['ID']?>" id="select_salon" required="true" <?=(empty($salon['METRO']['NAME']) ? 'style="text-indent: 35px;"' : '') ?>>
			                  ...
			                  <div id="salon_reserve_desc" data-id="<?=$arResult['SALON'][0]['ID']?>"><div class="salon-contacts"><span id="salon-adres" class="medi-color"><?=$arResult['SALON'][0]['ADDRESS']?></span></div><div class="salon-shedule"><span id="salon-shedule"><?=$arResult['SALON'][0]['SCHEDULE']?></span></div></div>
			                <script>
