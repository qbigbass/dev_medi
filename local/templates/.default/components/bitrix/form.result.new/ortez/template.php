				...
				<div class="form_row ">
		            <div class="form_cell">
						...
						<label class="webFormItemCaption" for="<?=$field_name;?>"><?=$arQuestion['CAPTION']?><?=($arQuestion['REQUIRED']=='Y' ? '<span class="webFormItemRequired">*</span>' : '');?></label>
		    				<input type="tel" placeholder="+7 (___) ___-__-__" id="<?=$field_name;?>"  <?=($arQuestion['REQUIRED']=='Y' ? 'required' : '');?> class="<?=($FIELD_SID == 'PHONE' ? 'addphonemask' : '');?> <?=$FIELD_SID?>_input"  name="<?=$field_name;?>" value="<?=(isset($_REQUEST[$field_name]) ? htmlspecialchars($_REQUEST[$field_name]) : ($_SESSION['lmx']['phone'] ? $_SESSION['lmx']['phone'] : ''));?>" maxlength="18">
		    				<p class="error_<?=$field_name;?>"><span style="display:none">Поле  не заполнено</span></p>
		    			</div>
		            </div>
		        </div>
